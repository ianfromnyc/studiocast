...
namespace studiocast::util {
    ...
    // Returns the PIDs (best-effort) that currently have an open fd referring to the same
    // underlying file/device node as `target`.
    //
    // This scans /proc/*/fd and may be limited by permissions (we ignore processes we can't inspect).
    std::vector<int> PidsWithOpenFile(const std::filesystem::path& target,
                                     const OpenFileScanOptions& opt,
                                     std::string* error);

    // Convenience helper: true if any process other than `exclude_pid` has `target` open.
