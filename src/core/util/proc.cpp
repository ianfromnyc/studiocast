...
bool GetFileId(const fs::path& p, FileId* out, std::string* error) {
    ...
    if (::stat(p.c_str(), &st) != 0) {
        if (error) {
            *error = "stat(" + p.string() + ") failed: " + std::string(std::strerror(errno));
        }
        return false;
    }
...
std::vector<int> PidsWithOpenFile(const fs::path& target,
    ...
    for (fs::directory_iterator it("/proc", procEc); it != fs::directory_iterator(); it.increment(procEc)) {
        if (procEc) {
            // /proc iteration failures shouldn't be fatal; just stop.
            if (error && error->empty()) *error = "Failed to iterate /proc: " + procEc.message();
            break;
        }

        const fs::directory_entry& ent = *it;
        if (!ent.is_directory(procEc)) continue;

        const std::string base = ent.path().filename().string();
        int pid = -1;
        if (!ParsePid(base, &pid)) continue;
        if (opt.exclude_pid > 0 && pid == opt.exclude_pid) continue;

        const fs::path fdDir = ent.path() / "fd";

        std::error_code fdEc;
        for (fs::directory_iterator fdit(fdDir, fdEc); fdit != fs::directory_iterator(); fdit.increment(fdEc)) {
            if (fdEc) {
...
bool AnyOtherProcessHasFileOpen(const fs::path& target, int exclude_pid, std::string* error) {
