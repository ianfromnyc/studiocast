...
public:
  ...
  bool Open(const std::filesystem::path &path, Scope scope,
  ...
  bool GetSymbol(const char *name, T *out, std::string *error_out) const {
    static_assert(std::is_pointer_v<T>,
                  "DynLib::GetSymbol<T> requires a pointer type.");
    if (!out) {
      if (error_out) {
...
private:
  static std::string DescribeOpenError(const std::filesystem::path &path,
                                       Scope scope, const char *dlerror_text);
  static std::string DescribeMissingSymbol(const std::filesystem::path &path,
                                           const char *symbol,
