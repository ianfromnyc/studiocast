...
namespace studiocast::util::json {
    ...
    const std::string* Value::AsString() const { return std::get_if<std::string>(&v); }
    ...
    namespace {
        struct Parser {
            ...
            bool ParseString(std::string* out) {
                SkipWs();
                if (Peek() != '"') {
                    if (err) *err = FormatError("expected string");
                    return false;
                }
                ++i;
                out->clear();
                while (i < s.size()) {
                    ...
                    if (static_cast<unsigned char>(c) < 0x20) {
                        if (err) *err = FormatError("control character in string");
                        return false;
                    }
                    if (c != '\\') {
                        out->push_back(c);
                    ...
                    if (i >= s.size()) {
                        if (err) *err = FormatError("unterminated escape sequence");
                        return false;
                    }
                    const char e = s[i++];
                    switch (e) {
                        ...
                        case 'u': {
                            if (i + 4 > s.size()) {
                                if (err) *err = FormatError("short \\u escape");
                                return false;
                            }
                            std::uint32_t cp = 0;
                            for (int k = 0; k < 4; ++k) {
                                ...
                                if (!n) {
                                    if (err) *err = FormatError("invalid hex in \\u escape");
                                    return false;
                                }
                            ...
                            if (cp >= 0xD800 && cp <= 0xDBFF) {
                                ...
                                if (i + 6 <= s.size() && s[i] == '\\' && s[i + 1] == 'u') {
                                    ...
                                    for (int k = 0; k < 4; ++k) {
                                        const auto n = HexNibble(s[i + static_cast<std::size_t>(k)]);
                                        if (!n) {
                                            if (err) *err = FormatError("invalid hex in surrogate escape");
                                            return false;
                                        }
                                    ...
                                    if (cp2 >= 0xDC00 && cp2 <= 0xDFFF) {
                                        ...
                                        if (err) *err = FormatError("invalid low surrogate");
                                        return false;
                                    }
                                } else {
                                    if (err) *err = FormatError("missing low surrogate");
                                    return false;
                                }
                            } else if (cp >= 0xDC00 && cp <= 0xDFFF) {
                                if (err) *err = FormatError("unexpected low surrogate");
                                return false;
                            }
                        ...
                        default:
                            if (err) *err = FormatError("unknown escape sequence");
                            return false;
                    }
                }
                if (err) *err = FormatError("unterminated string");
                return false;
            }
            ...
            bool ParseNumber(double* out) {
                ...
                if (Peek() == '0') {
                    ...
                    if (!std::isdigit(static_cast<unsigned char>(Peek()))) {
                        if (err) *err = FormatError("invalid number");
                        return false;
                    }
                ...
                if (Peek() == '.') {
                    ++i;
                    if (!std::isdigit(static_cast<unsigned char>(Peek()))) {
                        if (err) *err = FormatError("invalid fraction");
                        return false;
                    }
                    while (std::isdigit(static_cast<unsigned char>(Peek()))) ++i;
                }
                if (Peek() == 'e' || Peek() == 'E') {
                    ...
                    if (Peek() == '+' || Peek() == '-') ++i;
                    if (!std::isdigit(static_cast<unsigned char>(Peek()))) {
                        if (err) *err = FormatError("invalid exponent");
                        return false;
                    }
                    while (std::isdigit(static_cast<unsigned char>(Peek()))) ++i;
                }
                ...
                if (!endp || endp == tmp.c_str()) {
                    if (err) *err = FormatError("failed to parse number");
                    return false;
                }
                *out = val;
                return true;
            ...
            bool ParseValue(Value* out) {
                ...
                if (err) *err = FormatError("unexpected token");
                return false;
            }
            ...
            bool ParseObject(Value* out) {
                ...
                while (true) {
                    ...
                    SkipWs();
                    if (Consume('}')) break;
                    if (!Expect(',', "expected ',' or '}' in object")) return false;
                }
                *out = Value(std::move(obj));
                return true;
            }
            ...
            bool ParseArray(Value* out) {
                ...
                while (true) {
                    ...
                    SkipWs();
                    if (Consume(']')) break;
                    if (!Expect(',', "expected ',' or ']' in array")) return false;
                }
                *out = Value(std::move(arr));
                return true;
            }
    ...
    bool Parse(const std::string& text, Value* out, std::string* error) {
        ...
        Value v;
        if (!p.ParseValue(&v)) return false;
        p.SkipWs();
        if (p.i != text.size()) {
            if (error) *error = p.FormatError("trailing characters");
            return false;
        }
        *out = std::move(v);
        return true;
