...
DynLib::DynLib(DynLib &&other) noexcept { *this = std::move(other); }
...
bool DynLib::Open(const std::filesystem::path &path, Scope scope,
...
std::string DynLib::DescribeOpenError(const std::filesystem::path &path,
                                      Scope scope, const char *dlerror_text) {
  std::ostringstream oss;
...
std::string DynLib::DescribeMissingSymbol(const std::filesystem::path &path,
                                          const char *symbol,
                                          const char *dlerror_text) {
  std::ostringstream oss;
  oss << "dlsym failed: missing symbol '" << (symbol ? symbol : "") << "' in '"
      << path.string() << "'";
  if (path.is_absolute()) {
    oss << " (absolute path)";
  }
