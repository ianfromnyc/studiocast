...
bool QueryV4l2Cap(const std::string &devNode, std::string *outDriver,
                  std::string *outCard, std::string *outBusInfo) {
  ...
  if (outDriver)
    *outDriver = std::string(reinterpret_cast<const char *>(cap.driver));
  if (outCard)
    *outCard = std::string(reinterpret_cast<const char *>(cap.card));
  if (outBusInfo)
    *outBusInfo = std::string(reinterpret_cast<const char *>(cap.bus_info));
  ...
  if (rep.sys_video_class_present) {
    ...
    for (const auto &entry : fs::directory_iterator(sysDir, ec)) {
      ...
      if (QueryV4l2Cap(d.dev_node, &capDriver, &capCard, &capBus)) {
        if (d.name.empty())
          d.name = util::TrimCopy(capCard);
        if (d.driver.empty())
          d.driver = util::TrimCopy(capDriver);
