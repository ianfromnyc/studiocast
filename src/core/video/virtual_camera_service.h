...
namespace studiocast::video {
    ...
    struct VirtualCameraServiceConfig {
        ...
        // After the last consumer disconnects, we wait this long before stopping the pipeline.
        // This avoids rapid start/stop flapping when apps probe cameras.
        int stop_grace_ms = 1000;

        // If true, the pipeline runs even when there are no consumers of the virtual camera.
        // Useful for debugging or when you want the virtual camera to always be "hot".
    ...
    // Supervises the camera pipeline and starts/stops it based on whether any process is
    // consuming the v4l2loopback device (consumer-driven start/stop).
    //
    // This lives in studiocast_core (no Qt deps) so it can be reused by the GUI or a daemon.
    ...
    public:
        ...
        // Starts the supervisor thread (does not necessarily start the pipeline immediately).
    ...
    private:
        ...
        static bool NeedsPipelineRestart(const CameraPipelineConfig& a, const CameraPipelineConfig& b);
