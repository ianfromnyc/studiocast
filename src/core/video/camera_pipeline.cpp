...
bool CameraPipeline::OpenOutputLocked(const std::string& outDev,
  ...
  if (writer_.IsOpen() && writer_device_ == outDev) {
    ...
    // Idle keep-alive path (strict_fps=false): keep the existing producer FD alive.
    //
    ...
    // and renegotiate here, we can hit a window where VIDIOC_S_FMT/G_FMT returns
    // EINVAL, which manifests as start/stop flapping when the GUI preview is
    // restarted after the camera fully idles.
    //
...
bool CameraPipeline::EnsureOutputOpen(const CameraPipelineConfig& cfg, std::string* error) {
  ...
  const auto write_keepalive = [&](bool force, std::string* outErr) -> bool {
    if (!writer_.IsOpen())
      return true;
    if (!force && last_keepalive_frame_at_ != std::chrono::steady_clock::time_point{} &&
