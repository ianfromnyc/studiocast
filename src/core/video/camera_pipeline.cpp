...
bool CameraPipeline::EnsureOutputOpen(const CameraPipelineConfig &cfg,
  ...
  // the device is typically only considered a "real" capture source once a
  // producer has set a format and provided at least one frame. Only seed a
  // frame when we actually opened/renegotiated the output. If we reuse an
  // already-open writer, re-seeding would overwrite the live stream and
  // manifest as periodic black flashes.
  if (opened_or_renegotiated && writer_.IsOpen()) {
    ...
    if (size > 0) {
      ...
      if (!writer_.WriteFrame(buf.data(), buf.size(), &werr)) {
        if (error)
          *error = "Output opened, but failed to write initial frame: " + werr;
        return false;
      }
    }
  }
...
void CameraPipeline::ThreadMain(CameraPipelineConfig cfg) {
  ...
  struct OpenCudaAutoFrameContext {
    ...
    bool ApplyRgbInPlace(
      ...
      if (!EnsureInitialized(width, height, fx,
                             /*require_matte_tracking=*/!have_faces, &err)) {
      ...
      if (have_faces) {
        ...
        const studiocast::open_video::FaceDetection *best =
            &face_detections->front();
        ...
        box_px = studiocast::maxine::effects::RectF{new_x, new_y, expand_w,
                                                    expand_h};
        found = true;
        ...
        if (matte->GetAlphaCpuForFrame(rgb, stride, capture_sequence, width,
                                       ...
                                       &alpha_h, &alpha_err) &&
            alpha_cpu) {
          found = ComputeMatteBoxPxFromAlpha(*alpha_cpu, alpha_w, alpha_h,
                                             width, height, &box_px);
      ...
      if (found) {
        target_crop = studiocast::maxine::effects::ArAutoFrameTracker::
            ComputeTargetCropFromBoxPx(box_px, width, height, out_aspect,
                                       knobs);
      } else {
        const float strength01 =
            std::clamp(knobs.strength / 100.0f, 0.0f, 1.0f);
      ...
      if (std::abs(crop_smoothed_px.x) <= kEpsPx &&
          ...
          std::abs(crop_smoothed_px.w - width) <= kEpsPx &&
          std::abs(crop_smoothed_px.h - height) <= kEpsPx) {
        return true;
      }
  ...
  auto rebuildChain = [&](const studiocast::video::effects::
    ...
    if (vb_requested) {
      ...
      if (engine_open_cuda) {
        // Open source: prefer Open Video (YuNet) face tracking when available,
    ...
    bool fx_failed = false;
    {
      ...
      auto apply_stage = [&](const std::string &stage_id) {
        ...
        if (stage_id ==
          ...
          if (have_open_video_video_denoise) {
            std::string ov_err;
            if (!open_video_fastdvdnet.ApplyRgbInPlace(
