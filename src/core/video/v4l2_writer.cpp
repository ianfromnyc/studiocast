...
// report it inverted (as FPS). Try the canonical 1/fps first, then fall back to fps/1
// if the driver appears to interpret the fraction as FPS.
auto SetParm = [&](int num, int den) {
  v4l2_streamparm sp{};
  sp.type = (res.chosen_buf_type != 0) ? res.chosen_buf_type
...
auto GetParm = [&](int* outNum, int* outDen, __u32* outType) -> bool {
  v4l2_streamparm gp{};
  gp.type = (res.chosen_buf_type != 0) ? res.chosen_buf_type
                                       : static_cast<__u32>(V4L2_BUF_TYPE_VIDEO_OUTPUT);
  if (IoctlRetry(fd, VIDIOC_G_PARM, &gp) != 0) return false;
  const v4l2_fract tpf = IsOutputBufType(gp.type) ? gp.parm.output.timeperframe
                                                  : gp.parm.capture.timeperframe;
  if (outNum) *outNum = static_cast<int>(tpf.numerator);
  if (outDen) *outDen = static_cast<int>(tpf.denominator);
  if (outType) *outType = gp.type;
  return true;
};
...
if (GetParm(&qnum, &qden, &qtype)) {
  ...
  if (best.used_fps_fraction) {
    ...
    if (GetParm(&qnum2, &qden2, &qtype2)) {
      ...
      if (DebugV4l2Fps()) {
        std::ostringstream oss;
        oss << "writer " << device << " G_PARM(after alt set, type=" << BufTypeName(qtype2) << ")"
            << " raw=" << qnum2 << "/" << qden2
...
bool V4l2Writer::Open(const std::string& device,
  ...
  // Prefer O_WRONLY (important for exclusive_caps setups and avoids grabbing the read side).
  if (TryOpenNegotiate(device, O_WRONLY, width, height, fps, fmt, &fd, &a, &err1)) {
    fd_ = fd;
    actual_ = a;
    return true;
  }
  ...
  if (TryOpenNegotiate(device, O_RDWR, width, height, fps, fmt, &fd, &a, &err2)) {
    fd_ = fd;
    actual_ = a;
    return true;
  }
