...
// Every entry point below takes `src_stride` as the bytes each source row
// holds, not only as the step between rows. A captured YUYV row can be packed
// to width * 2 bytes, which for an odd width stops after the U byte of the
// last pixel. The converters read the V byte of that pixel only when the row
// is long enough to hold it, and use neutral chroma when it is not, so a
// packed row is never read past its end. A sub-row call must therefore pass
// the bytes that are left in the row, not the stride of the whole row.
