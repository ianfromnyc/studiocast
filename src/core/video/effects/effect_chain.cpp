...
std::string EffectChain::BackendSummary() const {
  ...
  for (const auto &fx : effects_) {
    ...
    if (!first)
      oss << ",";
    first = false;
    // User-facing diagnostics: avoid implying CPU effect backends are
    // supported. Some lightweight tail operations (e.g. `mirror`) are still
    // host-side, but we present them as "builtin" rather than "cpu".
    const std::string backend =
        fx->Backend() ? std::string(fx->Backend()) : std::string();
