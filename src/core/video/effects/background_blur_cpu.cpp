...
void BackgroundBlurCpuEffect::Apply(const Rgb24FrameView &frame,
                                    EffectContext * /*ctx*/) {
  if (!frame.Valid())
    return;
  ...
  if (blurred_.size() != tightSize)
    blurred_.assign(tightSize, 0);
  ...
  for (int y = 0; y < h; ++y) {
    ...
    std::uint8_t *dstRow =
        blurred_.data() + static_cast<std::size_t>(y) * tightStride;
