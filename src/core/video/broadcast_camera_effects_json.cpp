...
bool ParseRootObject(const Value &root, const Value::Object **out,
  ...
  if (!obj0) {
    if (error)
      *error = "effects patch JSON must be an object";
    return false;
  }
...
bool ApplyVirtualBackgroundEffectPatch(VirtualBackgroundMode mode,
  ...
  bool enabled = false;
  if (!TryGetBool(patch, std::string(effects::contract::param::kEnabled),
                  &found, &enabled, error))
    return false;
  if (found) {
    if (enabled) {
  ...
  std::string mid;
  if (!TryGetString(patch, std::string(effects::contract::param::kModelId),
                    &found, &mid, error))
    return false;
  if (found)
    fx->virtual_background.model_id = mid;

  int strength = 0;
  if (!TryGetInt(patch, std::string(effects::contract::param::kStrength),
                 &found, &strength, error))
    return false;
  if (found) {
    fx->virtual_background.strength =
        ClampInt(strength, effects::contract::kVbStrengthMin,
                 effects::contract::kVbStrengthMax);
  }

  std::string removeColor;
  if (!TryGetString(patch,
                    std::string(effects::contract::param::kVbRemoveColor),
                    &found, &removeColor, error))
    return false;
  if (found) {
    std::uint32_t rgb = 0;
    std::string err;
    if (!ParseHexColorRgb(removeColor, &rgb, &err)) {
      if (error)
        *error = "invalid remove_color: " + err;
      return false;
    }
  ...
  if (!TryGetInt(patch, std::string(effects::contract::param::kGreenscreenMode),
                 &found, &gsm, error))
    return false;
  if (found)
  ...
  if (!TryGetBool(patch,
                  std::string(effects::contract::param::kGreenscreenTemporal),
                  &found, &gst, error))
    return false;
  if (found)
    fx->virtual_background.greenscreen_temporal = gst;

  std::string rp;
  if (!TryGetString(patch,
                    std::string(effects::contract::param::kVbReplacePath),
                    &found, &rp, error))
    return false;
  if (found)
...
std::string BroadcastCameraEffectsContractToJson(
  ...
  const bool vbBlur =
      effects.virtual_background.mode == VirtualBackgroundMode::blur;
  const bool vbRemove =
      effects.virtual_background.mode == VirtualBackgroundMode::remove;
  const bool vbReplace =
  ...
  oss << "\"" << kVbRemoveColor << "\":\""
      ...
      << "\",";
  oss << "\"" << kGreenscreenMode
      << "\":" << effects.virtual_background.greenscreen_mode << ',';
  oss << "\"" << kGreenscreenTemporal << "\":"
  ...
  oss << "\"" << kEnabled
      << "\":" << (effects.auto_frame.enabled ? "true" : "false") << ',';
  ...
  oss << "\"" << kEnabled
      << "\":" << (effects.eye_contact.enabled ? "true" : "false") << ',';
  oss << "\"" << kStrength << "\":" << effects.eye_contact.strength << ',';
  oss << "\"" << kLookAwayEnabled
  ...
  oss << "\"" << kIntensity << "\":" << effects.virtual_key_light.intensity
      << ',';
  oss << "\"" << kTemperaturePreset << "\":\""
      << TemperaturePresetToString(effects.virtual_key_light.temperature_preset)
      << "\",";
  oss << "\"" << kDirectionPanDegrees
  ...
  oss << "\"" << kEnabled
      << "\":" << (effects.vignette.enabled ? "true" : "false") << ',';
  oss << "\"" << kIntensity << "\":" << effects.vignette.intensity << ',';
  oss << "\"" << kCenterOnTrackedFace
...
bool ApplyBroadcastCameraEffectsPatchJson(
  ...
  if (!ParseRootObject(root, &obj, error))
    return false;
  ...
  if (const auto *mo = GetObj(
          *obj, std::string(effects::contract::kEffectIdMirror), error)) {
    bool en = false;
    if (!TryGetBool(*mo, std::string(effects::contract::param::kEnabled),
                    &found, &en, error))
      return false;
    if (found)
      effects->mirror = en;
  ...
  if (const auto *vb = GetObj(
          *obj, std::string(effects::contract::kEffectIdVirtualBackgroundBlur),
          error)) {
    if (!ApplyVirtualBackgroundEffectPatch(VirtualBackgroundMode::blur, *vb,
                                           effects, error))
      return false;
  }
  if (const auto *vb = GetObj(
          *obj,
          std::string(effects::contract::kEffectIdVirtualBackgroundRemove),
          error)) {
    if (!ApplyVirtualBackgroundEffectPatch(VirtualBackgroundMode::remove, *vb,
                                           effects, error))
      return false;
  }
  if (const auto *vb = GetObj(
          *obj,
          std::string(effects::contract::kEffectIdVirtualBackgroundReplace),
          error)) {
    if (!ApplyVirtualBackgroundEffectPatch(VirtualBackgroundMode::replace, *vb,
                                           effects, error))
      return false;
  }
  ...
  if (const auto *vbo = GetObj(*obj, "virtual_background", error)) {
    std::string mode;
    if (!TryGetString(*vbo, "mode", &found, &mode, error))
      return false;
    if (found) {
      if (mode == "auto_frame") {
        ...
        if (!ParseVirtualBackgroundMode(mode, &m)) {
          if (error)
            *error = "unknown virtual_background.mode '" + mode + "'";
          return false;
        }
    ...
    int strength = 0;
    if (!TryGetInt(*vbo, "strength", &found, &strength, error))
      return false;
    if (found) {
      effects->virtual_background.strength =
          ClampInt(strength, effects::contract::kVbStrengthMin,
                   effects::contract::kVbStrengthMax);
    }

    std::string removeColor;
    if (!TryGetString(*vbo, "remove_color", &found, &removeColor, error))
      return false;
    if (found) {
      std::uint32_t rgb = 0;
      std::string err;
      if (!ParseHexColorRgb(removeColor, &rgb, &err)) {
        if (error)
          *error = "invalid virtual_background.remove_color: " + err;
        return false;
      }
    ...
    std::string rp;
    if (!TryGetString(*vbo, "replace_path", &found, &rp, error))
      return false;
    if (found)
      effects->virtual_background.replace_path = rp;

    int gsm = 0;
    if (!TryGetInt(*vbo, "greenscreen_mode", &found, &gsm, error))
      return false;
    if (found)
      effects->virtual_background.greenscreen_mode =
          static_cast<std::uint32_t>(std::max(0, gsm));
    ...
    bool gst = false;
    if (!TryGetBool(*vbo, "greenscreen_temporal", &found, &gst, error))
      return false;
    if (found)
      effects->virtual_background.greenscreen_temporal = gst;
  }

  // Auto frame.
  if (const auto *af = GetObj(
          *obj, std::string(effects::contract::kEffectIdAutoFrame), error)) {
    bool en = effects->auto_frame.enabled;
    if (!TryGetBool(*af, std::string(effects::contract::param::kEnabled),
                    &found, &en, error))
      return false;
    if (found) {
    ...
    if (!TryGetInt(*af, std::string(effects::contract::param::kStrength),
                   &found, &strength, error))
      return false;
    if (found)
      effects->auto_frame.strength = ClampInt(strength, 0, 100);

    int smoothing = effects->auto_frame.smoothing;
    if (!TryGetInt(*af, std::string(effects::contract::param::kSmoothing),
                   &found, &smoothing, error))
      return false;
    if (found)
      effects->auto_frame.smoothing = ClampInt(smoothing, 0, 100);

    float headroom = effects->auto_frame.headroom;
    if (!TryGetFloat(*af, std::string(effects::contract::param::kHeadroom),
                     &found, &headroom, error))
      return false;
    if (found)
      effects->auto_frame.headroom = ClampFloat(headroom, 0.0f, 1.0f);

    std::string modelId = effects->auto_frame.model_id;
    if (!TryGetString(*af, std::string(effects::contract::param::kModelId),
                      &found, &modelId, error))
      return false;
    if (found)
      effects->auto_frame.model_id = modelId;
  }
  if (const auto *af = GetObj(*obj, "auto_frame", error)) {
    bool en = effects->auto_frame.enabled;
    if (!TryGetBool(*af, "enabled", &found, &en, error))
      return false;
    if (found) {
      effects->auto_frame.enabled = en;
      if (en)
    ...
    int strength = effects->auto_frame.strength;
    if (!TryGetInt(*af, "strength", &found, &strength, error))
      return false;
    if (found)
      effects->auto_frame.strength = ClampInt(strength, 0, 100);

    int smoothing = effects->auto_frame.smoothing;
    if (!TryGetInt(*af, "smoothing", &found, &smoothing, error))
      return false;
    if (found)
      effects->auto_frame.smoothing = ClampInt(smoothing, 0, 100);

    float headroom = effects->auto_frame.headroom;
    if (!TryGetFloat(*af, "headroom", &found, &headroom, error))
      return false;
    if (found)
      effects->auto_frame.headroom = ClampFloat(headroom, 0.0f, 1.0f);

    std::string modelId = effects->auto_frame.model_id;
    if (!TryGetString(*af, "model_id", &found, &modelId, error))
      return false;
    if (found)
      effects->auto_frame.model_id = modelId;
  }
  ...
  // Eye contact.
  if (const auto *ec = GetObj(
          *obj, std::string(effects::contract::kEffectIdEyeContact), error)) {
    bool en = effects->eye_contact.enabled;
    if (!TryGetBool(*ec, std::string(effects::contract::param::kEnabled),
                    &found, &en, error))
      return false;
    if (found)
      effects->eye_contact.enabled = en;

    int strength = effects->eye_contact.strength;
    if (!TryGetInt(*ec, std::string(effects::contract::param::kStrength),
                   &found, &strength, error))
      return false;
    if (found)
      effects->eye_contact.strength = ClampInt(strength, 0, 100);

    bool lookAway = effects->eye_contact.look_away_enabled;
    if (!TryGetBool(*ec,
                    std::string(effects::contract::param::kLookAwayEnabled),
                    &found, &lookAway, error))
      return false;
    if (found)
      effects->eye_contact.look_away_enabled = lookAway;

    std::string modelId = effects->eye_contact.model_id;
    if (!TryGetString(*ec, std::string(effects::contract::param::kModelId),
                      &found, &modelId, error))
      return false;
    if (found)
      effects->eye_contact.model_id = modelId;
  }
  if (const auto *ec = GetObj(*obj, "eye_contact", error)) {
    bool en = effects->eye_contact.enabled;
    if (!TryGetBool(*ec, "enabled", &found, &en, error))
      return false;
    if (found)
      effects->eye_contact.enabled = en;

    int strength = effects->eye_contact.strength;
    if (!TryGetInt(*ec, "strength", &found, &strength, error))
      return false;
    if (found)
      effects->eye_contact.strength = ClampInt(strength, 0, 100);

    bool lookAway = effects->eye_contact.look_away_enabled;
    if (!TryGetBool(*ec, "look_away_enabled", &found, &lookAway, error))
      return false;
    if (found)
      effects->eye_contact.look_away_enabled = lookAway;

    std::string modelId = effects->eye_contact.model_id;
    if (!TryGetString(*ec, "model_id", &found, &modelId, error))
      return false;
    if (found)
      effects->eye_contact.model_id = modelId;
  }
  ...
  // Noise removal.
  if (const auto *dn = GetObj(
          *obj, std::string(effects::contract::kEffectIdVideoNoiseRemoval),
          error)) {
    bool en = effects->video_noise_removal.enabled;
    if (!TryGetBool(*dn, std::string(effects::contract::param::kEnabled),
                    &found, &en, error))
      return false;
    if (found)
      effects->video_noise_removal.enabled = en;

    int strength = effects->video_noise_removal.strength;
    if (!TryGetInt(*dn, std::string(effects::contract::param::kStrength),
                   &found, &strength, error))
      return false;
    if (found)
      effects->video_noise_removal.strength = ClampInt(strength, 0, 100);

    std::string modelId = effects->video_noise_removal.model_id;
    if (!TryGetString(*dn, std::string(effects::contract::param::kModelId),
                      &found, &modelId, error))
      return false;
    if (found)
      effects->video_noise_removal.model_id = modelId;
  }
  if (const auto *dn = GetObj(*obj, "video_noise_removal", error)) {
    bool en = effects->video_noise_removal.enabled;
    if (!TryGetBool(*dn, "enabled", &found, &en, error))
      return false;
    if (found)
      effects->video_noise_removal.enabled = en;

    int strength = effects->video_noise_removal.strength;
    if (!TryGetInt(*dn, "strength", &found, &strength, error))
      return false;
    if (found)
      effects->video_noise_removal.strength = ClampInt(strength, 0, 100);

    std::string modelId = effects->video_noise_removal.model_id;
    if (!TryGetString(*dn, "model_id", &found, &modelId, error))
      return false;
    if (found)
      effects->video_noise_removal.model_id = modelId;
  }
  ...
  // Virtual key light.
  if (const auto *vkl =
          GetObj(*obj, std::string(effects::contract::kEffectIdVirtualKeyLight),
                 error)) {
    bool en = effects->virtual_key_light.enabled;
    if (!TryGetBool(*vkl, std::string(effects::contract::param::kEnabled),
                    &found, &en, error))
      return false;
    if (found)
      effects->virtual_key_light.enabled = en;

    int intensity = effects->virtual_key_light.intensity;
    if (!TryGetInt(*vkl, std::string(effects::contract::param::kIntensity),
                   &found, &intensity, error))
      return false;
    if (found)
      effects->virtual_key_light.intensity = ClampInt(intensity, 0, 100);

    std::string preset;
    if (!TryGetString(*vkl,
                      std::string(effects::contract::param::kTemperaturePreset),
                      &found, &preset, error))
      return false;
    if (found) {
    ...
    if (!TryGetInt(*vkl,
                   std::string(effects::contract::param::kDirectionPanDegrees),
                   &found, &pan, error))
      return false;
    if (found) {
      effects->virtual_key_light.direction_pan_degrees =
          ClampInt(pan, effects::contract::kVirtualKeyLightPanMin,
                   effects::contract::kVirtualKeyLightPanMax);
    }

    std::string hp;
    if (!TryGetString(*vkl, std::string(effects::contract::param::kHdriPath),
                      &found, &hp, error))
      return false;
    if (found)
      effects->virtual_key_light.hdri_path = hp;
  }
  if (const auto *vkl = GetObj(*obj, "virtual_key_light", error)) {
    bool en = effects->virtual_key_light.enabled;
    if (!TryGetBool(*vkl, "enabled", &found, &en, error))
      return false;
    if (found)
      effects->virtual_key_light.enabled = en;

    int intensity = effects->virtual_key_light.intensity;
    if (!TryGetInt(*vkl, "intensity", &found, &intensity, error))
      return false;
    if (found)
      effects->virtual_key_light.intensity = ClampInt(intensity, 0, 100);

    // Accept preset as either a numeric code (0..2) or a string
    // (neutral/warm/cool) to avoid conflicts with the contract JSON form.
    if (const Value *tp = Find(*vkl, "temperature_preset")) {
      if (const auto *s = tp->AsString()) {
        effects->virtual_key_light.temperature_preset =
            TemperaturePresetFromString(*s);
        effects->virtual_key_light.temperature =
            *KelvinFromPreset(effects->virtual_key_light.temperature_preset);
        ...
        if (error)
          *error = "expected string or number for 'temperature_preset'";
        return false;
      }
    }

    int pan = effects->virtual_key_light.direction_pan_degrees;
    if (!TryGetInt(*vkl, "direction_pan_degrees", &found, &pan, error))
      return false;
    if (found) {
      effects->virtual_key_light.direction_pan_degrees =
          ClampInt(pan, effects::contract::kVirtualKeyLightPanMin,
    ...
    std::string hp;
    if (!TryGetString(*vkl, "hdri_path", &found, &hp, error))
      return false;
    if (found)
      effects->virtual_key_light.hdri_path = hp;
  }
  ...
  // Vignette.
  if (const auto *vg = GetObj(
          *obj, std::string(effects::contract::kEffectIdVignette), error)) {
    bool en = effects->vignette.enabled;
    if (!TryGetBool(*vg, std::string(effects::contract::param::kEnabled),
                    &found, &en, error))
      return false;
    if (found)
      effects->vignette.enabled = en;

    int intensity = effects->vignette.intensity;
    if (!TryGetInt(*vg, std::string(effects::contract::param::kIntensity),
                   &found, &intensity, error))
      return false;
    if (found)
      effects->vignette.intensity = ClampInt(intensity, 0, 100);

    bool center = effects->vignette.center_on_tracked_face;
    if (!TryGetBool(*vg,
                    std::string(effects::contract::param::kCenterOnTrackedFace),
                    &found, &center, error))
      return false;
    if (found)
      effects->vignette.center_on_tracked_face = center;
  }
  if (const auto *vg = GetObj(*obj, "vignette", error)) {
    bool en = effects->vignette.enabled;
    if (!TryGetBool(*vg, "enabled", &found, &en, error))
      return false;
    if (found)
      effects->vignette.enabled = en;

    int intensity = effects->vignette.intensity;
    if (!TryGetInt(*vg, "intensity", &found, &intensity, error))
      return false;
    if (found)
      effects->vignette.intensity = ClampInt(intensity, 0, 100);

    bool center = effects->vignette.center_on_tracked_face;
    if (!TryGetBool(*vg, "center_on_tracked_face", &found, &center, error))
      return false;
    if (found)
      effects->vignette.center_on_tracked_face = center;
  }
  ...
  effects->virtual_background.strength = ClampInt(
      effects->virtual_background.strength, effects::contract::kVbStrengthMin,
      effects::contract::kVbStrengthMax);
  effects->auto_frame.strength = ClampInt(
      effects->auto_frame.strength, effects::contract::kAutoFrameStrengthMin,
      effects::contract::kAutoFrameStrengthMax);
  effects->auto_frame.smoothing =
      ClampInt(effects->auto_frame.smoothing, 0, 100);
  effects->auto_frame.headroom = ClampFloat(
      effects->auto_frame.headroom, effects::contract::kAutoFrameHeadroomMin,
      effects::contract::kAutoFrameHeadroomMax);

  effects->eye_contact.strength =
      ClampInt(effects->eye_contact.strength, 0, 100);
  effects->video_noise_removal.strength =
      ClampInt(effects->video_noise_removal.strength, 0, 100);

  effects->virtual_key_light.intensity =
      ClampInt(effects->virtual_key_light.intensity, 0, 100);
  effects->virtual_key_light.temperature_preset =
      ClampInt(effects->virtual_key_light.temperature_preset, 0, 2);
  effects->virtual_key_light.direction_pan_degrees =
      ClampInt(effects->virtual_key_light.direction_pan_degrees,
...
ToBroadcastCameraEffects(const studiocast::video::CameraEffects &legacy) {
  ...
  fx.engine = (legacy.background_backend ==
               studiocast::video::effects::EffectBackend::maxine)
                  ? EffectsEnginePreference::maxine
                  : EffectsEnginePreference::auto_select;

  fx.virtual_background.strength =
      ClampInt(legacy.background_strength, effects::contract::kVbStrengthMin,
               effects::contract::kVbStrengthMax);
  fx.virtual_background.remove_color =
      RgbToHexColor(legacy.background_remove_color_rgb);
  ...
  fx.virtual_key_light.intensity =
      ClampInt(static_cast<int>(
                   std::lround(legacy.virtual_key_light.intensity * 100.0f)),
               0, 100);
  fx.virtual_key_light.temperature_preset =
      ClampInt(legacy.virtual_key_light.temperature_preset, 0, 2);
  fx.virtual_key_light.temperature =
      *KelvinFromPreset(fx.virtual_key_light.temperature_preset);
  fx.virtual_key_light.direction_pan_degrees =
      ClampInt(static_cast<int>(
                   std::lround(legacy.virtual_key_light.direction_pan_degrees)),
               effects::contract::kVirtualKeyLightPanMin,
               effects::contract::kVirtualKeyLightPanMax);
  fx.virtual_key_light.hdri_path = legacy.virtual_key_light.hdri_path.string();

  fx.vignette.enabled = legacy.vignette.enabled;
  fx.vignette.intensity = ClampInt(
      static_cast<int>(std::lround(legacy.vignette.intensity * 100.0f)), 0,
      100);
...
studiocast::video::CameraEffects ToLegacyCameraEffects(
  ...
  if (fx.auto_frame.enabled) {
    ...
    case VirtualBackgroundMode::none:
      out.background = studiocast::video::effects::BackgroundEffect::none;
      break;
    case VirtualBackgroundMode::blur:
      out.background = studiocast::video::effects::BackgroundEffect::blur;
      break;
    case VirtualBackgroundMode::remove:
      out.background = studiocast::video::effects::BackgroundEffect::remove;
      break;
    case VirtualBackgroundMode::replace:
      out.background = studiocast::video::effects::BackgroundEffect::replace;
      break;
    }
  }
  ...
  out.auto_frame.strength =
      ClampInt(fx.auto_frame.strength, effects::contract::kAutoFrameStrengthMin,
               effects::contract::kAutoFrameStrengthMax);
  out.auto_frame.smoothing = ClampInt(fx.auto_frame.smoothing, 0, 100);
  out.auto_frame.headroom = ClampFloat(
      fx.auto_frame.headroom, effects::contract::kAutoFrameHeadroomMin,
      effects::contract::kAutoFrameHeadroomMax);
  ...
  out.virtual_key_light.intensity = ClampFloat(
      static_cast<float>(ClampInt(fx.virtual_key_light.intensity, 0, 100)) /
          100.0f,
      0.0f, 1.0f);
  out.virtual_key_light.temperature_preset =
      ClampInt(fx.virtual_key_light.temperature_preset, 0, 2);
  out.virtual_key_light.direction_pan_degrees =
      static_cast<float>(ClampInt(fx.virtual_key_light.direction_pan_degrees,
                                  effects::contract::kVirtualKeyLightPanMin,
                                  effects::contract::kVirtualKeyLightPanMax));
  out.virtual_key_light.hdri_path = fx.virtual_key_light.hdri_path;

  out.vignette.enabled = fx.vignette.enabled;
  out.vignette.intensity = ClampFloat(
      static_cast<float>(ClampInt(fx.vignette.intensity, 0, 100)) / 100.0f,
      0.0f, 1.0f);
