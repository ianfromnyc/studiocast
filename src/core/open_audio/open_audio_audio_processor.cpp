...
// These are intentionally lightweight, dependency-free resamplers tuned for 3:1.
struct OpenAudioAudioProcessor::Decimator3 {
  explicit Decimator3(std::vector<float> taps) : h(std::move(taps)) {
    if (h.size() >= 2) hist.assign(h.size() - 1, 0.0f);
  }

  void Reset() { std::fill(hist.begin(), hist.end(), 0.0f); }

  bool Process(const float* in, std::size_t in_len, float* out, std::size_t out_len) {
    if (!in || !out) return false;
    if (h.empty()) return false;
    if ((in_len % 3) != 0) return false;
    const std::size_t expected_out = in_len / 3;
    if (out_len < expected_out) return false;
    ...
    if (hist_size > 0) {
      std::copy(tmp.end() - static_cast<std::ptrdiff_t>(hist_size), tmp.end(), hist.begin());
    }
    return true;
  }
...
struct OpenAudioAudioProcessor::Interpolator3 {
  explicit Interpolator3(std::vector<float> taps) {
    ...
    if (phase_len >= 2) hist.assign(phase_len - 1, 0.0f);
  }
  ...
  bool Process(const float* in, std::size_t in_len, float* out, std::size_t out_len) {
    if (!in || !out) return false;
    if (phase_len == 0) return false;
    if (out_len < in_len * 3) return false;

    // Polyphase interpolation:
    // y[3n + p] = 3 * sum_{k=0}^{P-1} h_p[k] * x[n-k]
    // where x is the 16k stream, y is 48k, and we scale by 3 to preserve DC gain.
      ...
      for (int p = 0; p < 3; ++p) {
        ...
        for (std::size_t k = 0; k < P; ++k) {
          ...
          if (idx >= 0) {
            ...
            const std::ptrdiff_t hidx = static_cast<std::ptrdiff_t>(hist.size()) + idx;
        ...
        out[n * 3 + static_cast<std::size_t>(p)] = static_cast<float>(3.0 * acc);
      }
    }
    ...
    if (hist.size() > 0) {
      if (in_len >= hist.size()) {
        ...
        std::move(hist.begin() + static_cast<std::ptrdiff_t>(in_len), hist.end(), hist.begin());
        std::copy(in, in + in_len, hist.begin() + static_cast<std::ptrdiff_t>(keep));
      }
    }
    return true;
  }
...
float StrengthCurve01(StrengthMode mode, float t01) {
  ...
  switch (mode) {
    case StrengthMode::kRoomEchoRemoval:
      // More aggressive early ramp: helps make dereverb/echo removal noticeable at mid strengths.
...
float WetMixFromCurve01(StrengthMode mode, float curve01) {
  // Wet/dry mixing is only used when the model does not expose a native "strength" input.
  // These min values are tuned to keep artifacts low while still allowing the user to dial down.
...
std::vector<float> MakeLowpassFir(int taps, double cutoff_norm) {
  ...
  if (taps <= 0) return {};
  if ((taps % 2) == 0) taps += 1;  // force odd length
  if (cutoff_norm <= 0.0 || cutoff_norm >= 0.5) return {};
  ...
  for (int n = 0; n < taps; ++n) {
    ...
    const double sinc = (x == 0.0) ? 1.0 : (std::sin(2.0 * kPi * cutoff_norm * x) / (kPi * x));
    const double ideal = 2.0 * cutoff_norm * sinc;

    // Hamming window.
    const double w = 0.54 - 0.46 * std::cos((2.0 * kPi * static_cast<double>(n)) / static_cast<double>(M));
  ...
  out.reserve(h.size());
  for (double v : h) out.push_back(static_cast<float>(v));
  return out;
}
...
bool GetStringArrayOptional(const studiocast::util::json::Value::Object& o,
  ...
  for (const auto& el : *a) {
    ...
    if (!s2) return Fail(error, std::string("model.json: field '") + key + "' array must contain strings");
    out->push_back(*s2);
  }
  return true;
}
...
bool ParseAuxStrengthInput(const studiocast::util::json::Value::Object& ioObj,
  ...
  if (it == auxObj->end()) return true;

  const auto& v = it->second;
  out->has_strength_input = true;

  if (const auto* sname = v.AsString()) {
    if (sname->empty()) return Fail(error, "model.json: onnx_io.aux_inputs.strength must be non-empty");
    out->strength_input.name = *sname;
  } else {
    const auto* sobj = v.AsObject();
    if (!sobj) return Fail(error, "model.json: onnx_io.aux_inputs.strength must be a string or an object");
    ...
    if (!nameStr || nameStr->empty()) {
      return Fail(error, "model.json: onnx_io.aux_inputs.strength.name must be non-empty");
    }
    ...
    if (rangeVal) {
      ...
      if (!lo || !hi) return Fail(error, "model.json: onnx_io.aux_inputs.strength.range must contain only numbers");
    ...
    if (shapeVal) {
      ...
      for (const auto& el : *a) {
        ...
        if (!n) return Fail(error, "model.json: onnx_io.aux_inputs.strength.shape must contain only integers");
        const int64_t d = static_cast<int64_t>(*n);
        if (static_cast<double>(d) != *n || d <= 0) {
          return Fail(error, "model.json: onnx_io.aux_inputs.strength.shape must contain only positive integers");
        }
  ...
  if (out->strength_input.shape.empty()) out->strength_input.shape.push_back(1);
...
bool ParseOnnxIo(const studiocast::util::json::Value::Object& obj,
  ...
  if (!ParseAuxStrengthInput(*ioObj, &out->onnx_io, error)) return false;
...
std::size_t FindNameIndex(const std::vector<std::string>& names, const std::string& name) {
...
bool DeriveAudioShape(const std::vector<int64_t>& ort_shape,
  ...
  if (ort_shape.size() == 1) {
    ...
    if (d0 > 0 && static_cast<std::size_t>(d0) != frame_samples) {
      return Fail(error, std::string("Open Audio: ") + what + " shape mismatch (expected " +
                           std::to_string(frame_samples) + ", got " + std::to_string(d0) + ")");
    }
    out_shape->push_back(static_cast<int64_t>(frame_samples));
    return true;
  }
  ...
  if (ort_shape.size() == 2) {
    ...
    if (b == -1) b = 1;
    if (b != 1) {
      return Fail(error, std::string("Open Audio: ") + what + " batch dimension must be 1 (or dynamic)");
    }
    if (n == -1) n = static_cast<int64_t>(frame_samples);
    if (n != static_cast<int64_t>(frame_samples)) {
      return Fail(error, std::string("Open Audio: ") + what + " time dimension mismatch (expected " +
                           std::to_string(frame_samples) + ", got " + std::to_string(n) + ")");
    }
  ...
  return Fail(error, std::string("Open Audio: unsupported ") + what + " rank (expected 1 or 2)");
}
...
bool DeriveScalarShape(const std::vector<int64_t>& ort_shape,
  ...
  for (const auto d : *out_shape) {
    if (d <= 0) return Fail(error, "Open Audio: strength input shape must be positive");
    if (prod > 1) break;
    prod *= d;
  }
  if (prod != 1) {
    return Fail(error, "Open Audio: strength input must be a scalar (product(shape)==1)");
  }
  return true;
}
...
bool ResolveFromPackDir(const fs::path& pack_dir, ResolvedOpenAudioModel* out, std::string* error) {
  ...
  if (sample_rate < 0) return Fail(error, "model.json: sample_rate must be >= 0");
  if (channels <= 0) return Fail(error, "model.json: channels must be >= 1");
...
bool ResolveOpenAudioModelForMicrophone(const studiocast::audio::effects::BroadcastAudioEffects& fx,
  ...
  if (out) {
    ...
    if (pack.has_onnx_io) {
      ...
      out->onnx_io.strength_input.min_value = pack.onnx_io.strength_input.min_value;
      out->onnx_io.strength_input.max_value = pack.onnx_io.strength_input.max_value;
...
bool OpenAudioAudioProcessor::InitializeBindings(std::string* error) {
  ...
  audio_input_name_ = (model_.has_onnx_io && !model_.onnx_io.audio_input.empty()) ? model_.onnx_io.audio_input
                                                                                   : si.input_names[0];
  audio_output_name_ =
      (model_.has_onnx_io && !model_.onnx_io.audio_output.empty()) ? model_.onnx_io.audio_output : si.output_names[0];

  // If sample rate / frame size metadata is missing (common when using a bare .onnx file),
  // attempt to infer the frame size from the ORT audio input shape.
  const bool need_infer_frame =
      (model_.sample_rate <= 0) && (!model_.has_onnx_io || model_.onnx_io.frame_samples <= 0);
  if (need_infer_frame) {
  ...
  if (model_.has_onnx_io && model_.onnx_io.has_strength_input && !model_.onnx_io.strength_input.name.empty()) {
    ...
    const std::vector<int64_t> ort_shape = (idx != static_cast<std::size_t>(-1) && idx < si.input_shapes.size())
                                               ? si.input_shapes[idx]
                                               : std::vector<int64_t>{};

    if (!DeriveScalarShape(ort_shape, model_.onnx_io.strength_input.shape, &strength_input_shape_, error)) return false;
  ...
  const std::vector<int64_t> in_shape =
      (in_idx != static_cast<std::size_t>(-1) && in_idx < si.input_shapes.size()) ? si.input_shapes[in_idx]
                                                                                  : std::vector<int64_t>{};
  if (!DeriveAudioShape(in_shape, model_frame_samples_, &audio_input_shape_, error, "audio input")) return false;

  const std::size_t out_idx = FindNameIndex(si.output_names, audio_output_name_);
  const std::vector<int64_t> out_shape =
      (out_idx != static_cast<std::size_t>(-1) && out_idx < si.output_shapes.size()) ? si.output_shapes[out_idx]
                                                                                    : std::vector<int64_t>{};
  if (!DeriveAudioShape(out_shape, model_frame_samples_, &audio_output_shape_, error, "audio output")) return false;
  ...
  if (model_.has_onnx_io && !model_.onnx_io.state_inputs.empty()) {
    ...
    for (const auto& n : si.input_names) {
      if (n == audio_input_name_) continue;
      if (has_strength_input_ && n == strength_input_name_) continue;
  ...
  if (model_.has_onnx_io && !model_.onnx_io.state_outputs.empty()) {
    ...
    for (const auto& n : si.output_names) {
      if (n == audio_output_name_) continue;
  ...
  if (!state_input_names_.empty()) {
    if (state_output_names_.size() != state_input_names_.size()) {
      return Fail(error,
                  "Open Audio: model exposes state inputs/outputs but they do not match (pack must declare onnx_io.state_inputs/state_outputs)");
    }
  }
  ...
  if (!state_input_names_.empty()) {
    ...
    for (std::size_t i = 0; i < nstates; ++i) {
      ...
      if (idx_in == static_cast<std::size_t>(-1) || idx_out == static_cast<std::size_t>(-1)) {
        return Fail(error, "Open Audio: state tensor name not found in ORT session I/O");
      }

      const std::vector<int64_t> shape_in =
          (idx_in < si.input_shapes.size()) ? si.input_shapes[idx_in] : std::vector<int64_t>{};
      const std::vector<int64_t> shape_out =
          (idx_out < si.output_shapes.size()) ? si.output_shapes[idx_out] : std::vector<int64_t>{};

      if (shape_in.empty() || shape_out.empty() || shape_in.size() != shape_out.size()) {
        return Fail(error, "Open Audio: unable to derive state tensor shapes (pack may need explicit shapes)");
      }
      ...
      for (std::size_t d = 0; d < shape.size(); ++d) {
        ...
        if (a == -1 && b == -1) a = 1, b = 1;
        if (a != b) {
          return Fail(error, "Open Audio: state tensor shape mismatch between input and output");
        }
        shape[d] = a;
      }
      ...
      for (const auto dim : shape) {
        if (dim <= 0) return Fail(error, "Open Audio: state tensor shape must be positive");
        prod *= dim;
      }
  ...
  if (model_sample_rate_ == 16000) {
    ...
    if (taps.empty() || (taps.size() % 3) != 0) {
      return Fail(error, "Open Audio: internal resampler initialization failed");
    }
    decim3_ = std::make_unique<Decimator3>(taps);
    interp3_ = std::make_unique<Interpolator3>(taps);
  } else if (model_sample_rate_ != 48000) {
    return Fail(error, "Open Audio: unsupported model sample_rate (only 16000 and 48000 are supported)");
  }

  // Build ORT input/output bindings (names + shapes; data pointers are refreshed in Process()).
  ort_inputs_.clear();
  ort_outputs_.clear();
...
void OpenAudioAudioProcessor::Reset() {
  ...
  if (decim3_) decim3_->Reset();
  if (interp3_) interp3_->Reset();
}
...
bool OpenAudioAudioProcessor::Process(const float* in,
  ...
  if (!sticky_warning_.empty()) {
    if (error) *error = sticky_warning_;
    sticky_warning_.clear();
  }

  // Fail open.
  if (!ort_session_active_ || model_disabled_ || ort_inputs_.empty() || ort_outputs_.empty()) {
  ...
  if (model_sample_rate_ == 16000) {
    ...
    if (frames != 480 || model_frame_samples_ != 160 || !decim3_ || !interp3_) {
      sticky_warning_ = "Open Audio: incompatible frame sizing for 48k<->16k resampling (expected 480->160).";
      std::copy_n(in, samples, out);
      return true;
    }
    if (!decim3_->Process(mono_in_.data(), frames, model_in_.data(), model_in_.size())) {
    ...
    if (frames != model_frame_samples_) {
      sticky_warning_ = "Open Audio: incompatible frame size (model expects " + std::to_string(model_frame_samples_) +
                        " samples, got " + std::to_string(frames) + "); bypassing.";
      std::copy_n(in, samples, out);
      return true;
    }
  ...
  if (!has_strength_input_) {
    ...
    const float s_val = strength_input_min_ + curve * (strength_input_max_ - strength_input_min_);
    if (!strength_input_buf_.empty()) strength_input_buf_[0] = s_val;
  }

  // Refresh state input/output pointers for ping-pong buffers.
  std::size_t base_in = 1;
  if (has_strength_input_) base_in += 1;
  ...
  if (!ort_session_active_->Run(ort_inputs_.data(), ort_inputs_.size(), ort_outputs_.data(), ort_outputs_.size(),
                                &ort_err)) {
    if (error && error->empty()) *error = std::string("Open Audio ORT run failed: ") + ort_err;
  ...
  if (model_sample_rate_ == 16000) {
    if (!interp3_->Process(model_out_.data(), model_out_.size(), mono_out_.data(), mono_out_.size())) {
