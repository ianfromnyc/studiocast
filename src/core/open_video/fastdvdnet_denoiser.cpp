...
int ParseSigmaFromIdFallback(const std::string& id) {
  ...
  if (pos == std::string::npos) return 25;
  int v = 0;
  bool have = false;
...
std::string FastDvdnetDenoiser::ChoosePreferredModelId(const ModelPackRegistry& reg) {
  ...
  if (it == tasks.end() || it->second.empty()) return {};

  auto has = [&](const char* id) {
    return std::find(it->second.begin(), it->second.end(), std::string(id)) != it->second.end();
  };

  if (has("fastdvdnet_sigma25")) return "fastdvdnet_sigma25";
  if (has("fastdvdnet_sigma15")) return "fastdvdnet_sigma15";
  if (has("fastdvdnet_sigma50")) return "fastdvdnet_sigma50";

  // Deterministic fallback: first model id for task.
  return it->second.front();
}

bool FastDvdnetDenoiser::LoadDefaultSigmaFromManifest(const std::filesystem::path& manifest_path, int* out_sigma) {
  if (out_sigma) *out_sigma = 25;
  if (manifest_path.empty()) return false;

  const auto textOpt = util::ReadTextFile(manifest_path.string());
  if (!textOpt.has_value()) return false;
...
bool FastDvdnetDenoiser::ResolveModelFromRegistry(const ModelPackRegistry& reg, LoadedModel* out, std::string* error) {
  ...
  if (onnx.empty()) {
    if (error) *error = "Open Video FastDVDnet: model pack declares no ONNX file.";
    return false;
  }
...
bool FastDvdnetDenoiser::EnsureSessionForModel(const LoadedModel& model, std::string* error) {
  if (error) error->clear();

  if (model.onnx.empty()) {
    if (error) *error = "Open Video FastDVDnet: model ONNX path is empty.";
    return false;
  }

  // Avoid reloading if already active.
  if (initialized_ && !active_model_id_.empty() && model.id == active_model_id_ && model.onnx == active_model_path_ &&
      ort_session_active_) {
...
bool FastDvdnetDenoiser::RefreshGeometry(int src_w, int src_h, std::string* error) {
  if (error) error->clear();

  if (src_w <= 0 || src_h <= 0) {
    if (error) *error = "Open Video FastDVDnet: invalid frame size.";
    return false;
  }
  ...
  if (new_proc_w == proc_w_ && new_proc_h == proc_h_ && src_w == src_w_ && src_h == src_h_) {
    return true;
  }
  ...
  const std::size_t plane = static_cast<std::size_t>(proc_w_) * static_cast<std::size_t>(proc_h_);
...
bool FastDvdnetDenoiser::DetectIoNames(std::string* error) {
  ...
  if (!ort_session_active_) {
    if (error) *error = "Open Video FastDVDnet: ORT session not initialized.";
    return false;
  }

  const auto& info = session_info_;
  if (info.input_names.size() < 2 || info.output_names.empty()) {
    if (error) *error = "Open Video FastDVDnet: unexpected ONNX IO count (need >=2 inputs, >=1 output).";
    return false;
  }
  ...
  for (std::size_t i = 0; i < info.input_shapes.size(); ++i) {
    ...
    if (s.size() == 4) {
      if (s[1] == 15) noisy_idx = static_cast<int>(i);
      if (s[1] == 1) noise_idx = static_cast<int>(i);
    }
  }
  if (noisy_idx < 0 || noise_idx < 0 || noisy_idx == noise_idx) {
    ...
    for (std::size_t i = 0; i < info.input_names.size(); ++i) {
      ...
      if (noisy_idx < 0 && (n == "noisy" || n.find("noisy") != std::string::npos)) noisy_idx = static_cast<int>(i);
      if (noise_idx < 0 && (n == "noise_map" || n.find("noise") != std::string::npos))
        noise_idx = static_cast<int>(i);
    }
  }
  if (noisy_idx < 0) noisy_idx = 0;
  if (noise_idx < 0) noise_idx = (noisy_idx == 0 ? 1 : 0);
  ...
  if (noisy_name_.empty() || noise_map_name_.empty() || denoised_name_.empty()) {
    if (error) *error = "Open Video FastDVDnet: failed to resolve IO tensor names.";
    return false;
  }
  return true;
}
...
bool FastDvdnetDenoiser::EnsureInitialized(int src_w, int src_h, std::string* error) {
  ...
  if (!EnsureSessionForModel(model, &sess_err)) {
    if (error) *error = sess_err;
    return false;
  }

  std::string geo_err;
  if (!RefreshGeometry(src_w, src_h, &geo_err)) {
    if (error) *error = geo_err;
    return false;
  }
...
void FastDvdnetDenoiser::PreprocessRgbToChwPadded(const std::uint8_t* rgb,
                                                  int width,
                                                  int height,
                                                  std::size_t stride,
  ...
  for (int y = 0; y < proc_h_; ++y) {
    ...
    for (int x = 0; x < proc_w_; ++x) {
      ...
      const std::size_t idx = static_cast<std::size_t>(y) * static_cast<std::size_t>(proc_w_) +
...
void FastDvdnetDenoiser::BuildNoisyTensorFromHistory(std::vector<float>* out_noisy) const {
  if (!out_noisy) return;
  const std::size_t plane = static_cast<std::size_t>(proc_w_) * static_cast<std::size_t>(proc_h_);
  ...
  auto idx_t = [&](int back) -> int {
    ...
    if (history_filled_ <= 0) return 0;
    const int newest = (history_write_idx_ - 1 + kHistoryFrames) % kHistoryFrames;
    if (back == 0) return newest;
    if (history_filled_ < back + 1) {
      // Not enough history; replicate oldest available.
      const int oldest = (history_write_idx_ - history_filled_ + kHistoryFrames * 8) % kHistoryFrames;
      return oldest;
    }
  ...
  const std::vector<const std::vector<float>*> frames = {f_t2, f_t1, f_t0, f_tp1, f_tp2};

  for (std::size_t fi = 0; fi < frames.size(); ++fi) {
    const auto* f = frames[fi];
    if (!f || f->size() < 3 * plane) continue;
...
void FastDvdnetDenoiser::EnsureNoiseMap(float sigma_over_255) {
  ...
  if (std::abs(v - last_noise_map_value_) < 1e-6f && !noise_map_tensor_.empty()) return;
  last_noise_map_value_ = v;
  std::fill(noise_map_tensor_.begin(), noise_map_tensor_.end(), v);
}

void FastDvdnetDenoiser::PostprocessToRgbInPlace(std::uint8_t* rgb,
                                                 int width,
                                                 int height,
                                                 std::size_t stride) const {
  if (!rgb) return;
  if (denoised_tensor_.empty()) return;

  const std::size_t plane = static_cast<std::size_t>(proc_w_) * static_cast<std::size_t>(proc_h_);
  if (denoised_tensor_.size() < 3 * plane) return;
  ...
  for (int y = 0; y < height; ++y) {
    ...
    const std::size_t base = static_cast<std::size_t>(y) * static_cast<std::size_t>(proc_w_);
...
bool FastDvdnetDenoiser::ApplyRgbInPlace(std::uint64_t capture_sequence,
  ...
  if (!rgb) {
    if (error) *error = "Open Video FastDVDnet: null RGB buffer.";
    return false;
  }
  ...
  if (!ort_session_active_) {
    if (error) *error = "Open Video FastDVDnet: ORT session missing.";
    return false;
  }
  ...
  if (history_.empty()) {
    ...
    const std::size_t plane = static_cast<std::size_t>(proc_w_) * static_cast<std::size_t>(proc_h_);
    history_.resize(kHistoryFrames);
    for (auto& f : history_) f.assign(3 * plane, 0.0f);
  }

  PreprocessRgbToChwPadded(rgb, width, height, stride, &history_[static_cast<std::size_t>(history_write_idx_)]);
  ...
  if (!ort_session_active_->Run(ort_inputs_.data(), ort_inputs_.size(), ort_outputs_.data(), ort_outputs_.size(),
    ...
    if (!using_cpu_fallback_ && ort_session_cuda_ && ort_session_cpu_ && session_info_.using_cuda) {
      ort_session_active_ = ort_session_cpu_.get();
      using_cpu_fallback_ = true;
      sticky_warning_ = "Open Video FastDVDnet: switched to CPU fallback after a CUDA runtime failure.";
      runtime_failures_ = 0;
    ...
    if (runtime_failures_ >= 3) {
      DisableAfterFailure("Open Video FastDVDnet: disabled after repeated runtime failures.");
    }
