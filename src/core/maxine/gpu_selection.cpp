...
std::string SelectedGpu::ComputeCapString() const {
    ...
    return std::to_string(compute_capability->first) + "." + std::to_string(compute_capability->second);
}

bool IsComputeCapabilitySupported(int major, int minor) {
    // Maxine VFX requires Tensor cores; in practice this means Turing (7.5) or newer.
    if (major > 7) {
        return true;
    }
    if (major == 7 && minor >= 5) {
...
GpuSelectionResult SelectGpu(const config::GpuSelection& policy) {
    ...
    if (policy.mode == config::GpuSelectionMode::Uuid) {
        for (const auto& gpu : result.all_gpus) {
            if (gpu.uuid == policy.uuid) {
                ...
                const auto [maj, min] = *gpu.compute_capability;
                if (!IsComputeCapabilitySupported(maj, min)) {
                    result.error = "Selected GPU compute capability " + gpu.ComputeCapString() + " is not supported by Maxine.";
                    return result;
                }
                result.selected = gpu;
                return result;
    ...
    if (!result.selected.has_value()) {
        result.error = "No supported NVIDIA GPU found (requires Turing/RTX class or newer).";
    }
    return result;
}
