...
fs::path ChooseRoot(const std::optional<fs::path> &env_override,
                    const fs::path &xdg_default, const fs::path &system_default,
  ...
  // Default to the XDG location even if missing, so callers can show a stable
  // hint.
  if (source_out)
    *source_out = "xdg";
  return xdg_default;
...
std::string SearchedDirsString(const std::vector<fs::path> &dirs) {
  ...
  s.reserve(dirs.size());
  for (const auto &d : dirs)
    s.push_back(d.string());
  return JoinStrings(s, ", ");
}

ComponentPaths
ResolveComponent(const std::string &component, const char *env_var,
                 const fs::path &xdg_default, const fs::path &system_default,
                 std::vector<std::string> library_names,
                 bool require_models_dir, bool require_features_dir,
  ...
  out.candidate_roots = {};
  if (env_override)
    out.candidate_roots.push_back(*env_override);
  ...
  out.searched_lib_dirs.insert(out.searched_lib_dirs.end(),
                               base_lib_dirs.begin(), base_lib_dirs.end());
...
MaxinePathsReport ResolveMaxinePaths() {
  ...
  rep.vfx =
      ResolveComponent("VFX", "STUDIOCAST_VFX_SDK_ROOT", util::DefaultVfxRoot(),
  ...
  rep.afx = ResolveComponent(
      ...
      fs::path("/usr/local/Audio_Effects_SDK"), {"libnv_audiofx.so"},
      /*require_models_dir=*/false,
      /*require_features_dir=*/true,
      /*extra_lib_dirs_relative=*/
      {fs::path("nvafx") / "lib", fs::path("nvafx") / "lib64"});
