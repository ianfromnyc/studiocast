...
std::vector<fs::path> CandidateLibDirs(const std::vector<fs::path>& roots) {
    ...
    for (const auto& root : roots) {
        ...
        // Some layouts put shared libraries under the root directly or standard lib dirs.
        dirs.push_back(root);
...
bool AfxApi::Initialize(const std::vector<std::filesystem::path>& sdk_roots, std::string* error_out) {
    initialized_ = false;
    library_path_.clear();
    lib_.Close();
    f_ = Functions{};
...
bool AfxApi::InitializeFromLibraryPath(const std::filesystem::path& library_path, std::string* error_out) {
    initialized_ = false;
    library_path_.clear();
    lib_.Close();
    f_ = Functions{};
...
bool AfxApi::InitializeImpl(const std::vector<fs::path>& sdk_roots, std::string* error_out) {
    ...
    auto found = FindLibWithSymbol(dirs, preferred, "NvAFX_CreateEffect", util::DynLib::Scope::Global, &last);
    if (!found) {
...
bool AfxApi::InitializeFromLibraryPathImpl(const fs::path& library_path, std::string* error_out) {
    std::string err;
...
bool AfxApi::LoadSymbols(std::string* error_out) {
    ...
    if (!req("NvAFX_CreateEffect", &f_.NvAFX_CreateEffect)) return false;
    if (!req("NvAFX_DestroyEffect", &f_.NvAFX_DestroyEffect)) return false;

    if (!req("NvAFX_SetU32", &f_.NvAFX_SetU32)) return false;
    // Optional list setter.
    (void)lib_.GetSymbol("NvAFX_SetU32List", &f_.NvAFX_SetU32List, nullptr);
    if (!req("NvAFX_SetFloat", &f_.NvAFX_SetFloat)) return false;
    if (!req("NvAFX_SetString", &f_.NvAFX_SetString)) return false;

    if (!req("NvAFX_GetU32", &f_.NvAFX_GetU32)) return false;
    // Optional list getter.
    (void)lib_.GetSymbol("NvAFX_GetU32List", &f_.NvAFX_GetU32List, nullptr);

    if (!req("NvAFX_Load", &f_.NvAFX_Load)) return false;
    if (!req("NvAFX_Run", &f_.NvAFX_Run)) return false;
