...
bool LooksLikeNvcvImageLibName(const fs::path& p) {
    const auto s = p.filename().string();
    if (s.find("nvcv") == std::string::npos && s.find("NVCV") == std::string::npos) {
        return false;
    }
    if (s.find("image") == std::string::npos && s.find("Image") == std::string::npos) {
        return false;
    }
...
NvcvApi::NvcvApi(NvcvApi&& other) noexcept {
    *this = std::move(other);
}
...
bool NvcvApi::Initialize(Requirement req, const std::vector<fs::path>& sdk_roots, std::string* error_out) {
    return InitializeImpl(req, sdk_roots, error_out);
}

bool NvcvApi::InitializeFromLibraryPath(Requirement req, const fs::path& library_path, std::string* error_out) {
    return InitializeFromLibraryPathImpl(req, library_path, error_out);
}

bool NvcvApi::InitializeImpl(Requirement req, const std::vector<fs::path>& sdk_roots, std::string* error_out) {
    initialized_ = false;
    library_path_.clear();
    f_ = Functions{};
    error_.clear();
    ...
    const char* probe_symbol = (req == Requirement::Minimal) ? "NvCVImage_Alloc" : "NvCVImage_Init";

    std::string last;
    auto found = FindLibWithSymbol(dirs, preferred_names, probe_symbol, util::DynLib::Scope::Global, &last);
    if (!found) {
...
bool NvcvApi::InitializeFromLibraryPathImpl(Requirement req, const fs::path& library_path, std::string* error_out) {
    initialized_ = false;
    library_path_.clear();
    f_ = Functions{};
    error_.clear();
...
bool NvcvApi::LoadSymbols(Requirement req, std::string* error_out) {
    ...
    if (req == Requirement::VfxCompat) {
        if (!load_required("NvCVImage_Init", &f_.NvCVImage_Init) ||
            ...
            !load_required("NvCVImage_CompositeOverConstant", &f_.NvCVImage_CompositeOverConstant)) {
            return false;
        }
    } else {
        load_optional("NvCVImage_Init", &f_.NvCVImage_Init);
        load_optional("NvCVImage_Realloc", &f_.NvCVImage_Realloc);
        load_optional("NvCVImage_CompositeOverConstant", &f_.NvCVImage_CompositeOverConstant);
    }

    // Optional error strings (may live here or in a parent SDK library; best effort).
