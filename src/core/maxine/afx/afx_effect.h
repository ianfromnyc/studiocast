...
struct AfxEffectConfig {
    ...
    // Examples: "denoiser", "dereverb", "dereverb_denoiser", "studio_voice_low_latency".
    std::string effect_selector;

    // Feature ID used to locate feature assets under `<AFX_ROOT>/features/<feature_id>/...`.
    // Examples: "denoiser", "dereverb", "dereverb_denoiser", "studio_voice".
    ...
    // Selected GPU compute capability (major, minor). Required when `model_path` is empty.
...
//  - If Studio Voice is enabled: select `studio_voice_low_latency` and disable denoiser/dereverb.
...
private:
    bool SetU32Any(NvAFX_Handle handle,
                   const char* what,
                   std::initializer_list<const char*> candidates,
                   std::uint32_t v,
                   std::string* error_out);
    bool SetFloatAny(NvAFX_Handle handle,
                     const char* what,
                     std::initializer_list<const char*> candidates,
                     float v,
                     std::string* error_out);
    bool SetStringAny(NvAFX_Handle handle,
                      const char* what,
                      std::initializer_list<const char*> candidates,
                      const std::string& v,
                      std::string* error_out);
