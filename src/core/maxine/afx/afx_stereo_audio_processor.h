...
//
// Strategy: convert stereo -> Mid/Side, run the effect on Mid only, preserve
// Side. This keeps the speaker path stereo-safe without needing a multi-channel
// AFX model.
//
// Notes:
//  - Assumes 48kHz / 10ms frames (480 samples).
//  - Fails open: on runtime failures it copies input to output and returns
//  true.
...
public:
  ...
  bool Process(const float *in, float *out, std::uint32_t frames,
    ...
    // Only special-case stereo. For other channel counts, fall back to
    // mono-style processing if possible.
    if (channels != 2) {
      if (!effect_) {
      ...
      std::string run_err;
      const bool ok =
          effect_->Run(in, out, static_cast<std::uint32_t>(samples), &run_err);
      if (!ok) {
        if (error && error->empty())
          *error = run_err.empty() ? "AFX Run failed" : run_err;
        std::copy_n(in, samples, out);
        return true;
      }
    ...
    // Scratch buffers: allocate once (best-effort). If frame size changes
    // unexpectedly, fail open rather than allocating in the real-time thread.
    if (mid_in_.size() < frames || mid_out_.size() < frames ||
        side_.size() < frames) {
      std::copy_n(in, samples, out);
      if (error && error->empty())
        *error = "AFX stereo adapter: unexpected frame size";
      return true;
    }
    ...
    std::string run_err;
    const bool ok =
        effect_->Run(mid_in_.data(), mid_out_.data(), frames, &run_err);
    if (!ok) {
      if (error && error->empty())
        *error = run_err.empty() ? "AFX Run failed" : run_err;
      std::copy_n(in, samples, out);
    ...
    // Safety post-processing (DC blocker + limiter) on the reconstructed
    // stereo.
    post_dsp_.ProcessInPlace(out, frames, channels);
