...
std::vector<fs::path> CandidateRoots(const std::vector<fs::path>& explicit_roots) {
...
bool ArApi::Initialize(const std::vector<fs::path>& sdk_roots, std::string* error_out) {
    initialized_ = false;
    library_path_.clear();
    lib_.Close();
    f_ = Functions{};
...
bool ArApi::InitializeFromLibraryPath(const fs::path& library_path, std::string* error_out) {
    initialized_ = false;
    library_path_.clear();
    lib_.Close();
    f_ = Functions{};
...
bool ArApi::InitializeImpl(const std::vector<fs::path>& sdk_roots, std::string* error_out) {
    ...
    auto found = FindLibWithSymbol(dirs, preferred, "NvAR_Create", util::DynLib::Scope::Global, &last);
    if (!found) {
...
bool ArApi::InitializeFromLibraryPathImpl(const fs::path& library_path, std::string* error_out) {
    std::string err;
...
bool ArApi::LoadSymbols(std::string* error_out) {
    ...
    auto load_optional = [&](const char* symbol, auto* out) { lib_.GetSymbol(symbol, out, nullptr); };

    if (!load_required("NvAR_Create", &f_.NvAR_Create) || !load_required("NvAR_Load", &f_.NvAR_Load) ||
        !load_required("NvAR_Run", &f_.NvAR_Run) || !load_required("NvAR_Destroy", &f_.NvAR_Destroy) ||
        !load_required("NvAR_SetU32", &f_.NvAR_SetU32) || !load_required("NvAR_SetS32", &f_.NvAR_SetS32) ||
        !load_required("NvAR_SetF32", &f_.NvAR_SetF32) || !load_required("NvAR_SetF64", &f_.NvAR_SetF64) ||
        !load_required("NvAR_SetU64", &f_.NvAR_SetU64) || !load_required("NvAR_SetObject", &f_.NvAR_SetObject) ||
        !load_required("NvAR_SetString", &f_.NvAR_SetString) ||
        !load_required("NvAR_SetF32Array", &f_.NvAR_SetF32Array) || !load_required("NvAR_GetU32", &f_.NvAR_GetU32) ||
        !load_required("NvAR_GetS32", &f_.NvAR_GetS32) || !load_required("NvAR_GetF32", &f_.NvAR_GetF32) ||
        !load_required("NvAR_GetF64", &f_.NvAR_GetF64) || !load_required("NvAR_GetU64", &f_.NvAR_GetU64) ||
        !load_required("NvAR_GetObject", &f_.NvAR_GetObject) || !load_required("NvAR_GetString", &f_.NvAR_GetString) ||
