...
namespace studiocast::maxine {
    ...
    std::optional<std::string> MaxineGpuArgFromComputeCap(const std::string& compute_cap) {
        ...
        if (maj == 7 && min == 5) return "t4";
        if (maj == 8 && min == 0) return "a100";
        if (maj == 8 && min == 6) return "a10";
        if (maj == 8 && min == 9) return "l4";
        if (maj == 9 && min == 0) return "h100";
        if (maj == 10 && min == 0) return "b100";
        if (maj == 12 && min == 0) return "b40";
