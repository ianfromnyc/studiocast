...
std::optional<SharedLibLoadResult> FindLibWithSymbol(const std::vector<fs::path>& lib_dirs,
                                                    const std::vector<std::string>& preferred_names,
                                                    const char* required_symbol,
                                                    util::DynLib::Scope scope,
                                                    std::string* last_error) {
    std::error_code ec;

    auto try_file = [&](const fs::path& full) -> std::optional<SharedLibLoadResult> {
...
bool VfxRuntime::Initialize(const config::GpuSelection& gpu_policy, std::string* error_out) {
    diag_ = VfxDiagnostics{};
    api_ = VfxApi{};

    auto load_required = [&](const util::DynLib& lib, const char* symbol, auto* out) -> bool {
        std::string err;
    ...
    auto load_optional = [&](const util::DynLib& lib, const char* symbol, auto* out) {
        lib.GetSymbol(symbol, out, nullptr);
    };
    ...
    // Load VFX library.
    {
        ...
        auto found = FindLibWithSymbol(lib_dirs, preferred, "NvVFX_CreateEffect", util::DynLib::Scope::Global, &last);
        if (!found) {
    ...
    // Load symbols (VFX)
    if (!load_required(vfx_lib_, "NvVFX_CreateEffect", &api_.NvVFX_CreateEffect) ||
        !load_required(vfx_lib_, "NvVFX_DestroyEffect", &api_.NvVFX_DestroyEffect) ||
        !load_required(vfx_lib_, "NvVFX_CudaStreamCreate", &api_.NvVFX_CudaStreamCreate) ||
        !load_required(vfx_lib_, "NvVFX_CudaStreamDestroy", &api_.NvVFX_CudaStreamDestroy) ||
        !load_required(vfx_lib_, "NvVFX_SetCudaStream", &api_.NvVFX_SetCudaStream) ||
        ...
        !load_required(vfx_lib_, "NvVFX_AllocateState", &api_.NvVFX_AllocateState) ||
        !load_required(vfx_lib_, "NvVFX_DeallocateState", &api_.NvVFX_DeallocateState) ||
        !load_required(vfx_lib_, "NvVFX_ResetState", &api_.NvVFX_ResetState) ||
        !load_required(vfx_lib_, "NvVFX_SetStateObjectHandleArray", &api_.NvVFX_SetStateObjectHandleArray)) {
        return false;
    }
    ...
    load_optional(vfx_lib_, "NvCV_GetErrorStringFromCode", &api_.NvCV_GetErrorStringFromCode);
    ...
    // NvCVImage symbols might live in the VFX library or in a separate library.
    {
        ...
        const bool loaded_from_vfx =
            nvcv_api_.InitializeFromLibraryPath(NvcvApi::Requirement::VfxCompat, diag_.vfx_library, &err);

        // 2) Otherwise, locate the canonical NvCVImage library (commonly libnvcvimage.so).
        if (!loaded_from_vfx) {
            if (!nvcv_api_.Initialize(NvcvApi::Requirement::VfxCompat, {diag_.sdk_root}, &err)) {
                diag_.error = err;
                if (error_out) {
                    *error_out = diag_.error;
                }
        ...
        api_.NvCVImage_CompositeOverConstant = nvcv_api_.f().NvCVImage_CompositeOverConstant;
        api_.NvCVImage_Composite = nvcv_api_.f().NvCVImage_Composite;

        if (!api_.NvCV_GetErrorStringFromCode) {
            api_.NvCV_GetErrorStringFromCode = nvcv_api_.f().NvCV_GetErrorStringFromCode;
        }
    }
    ...
    if (cudaSetDevice) {
        ...
        if (gpu_index != 0) {
            diag_.error =
                "CUDA runtime (libcudart) not found; cannot select a non-zero GPU index reliably.";
            if (error_out) {
                *error_out = diag_.error;
            }
            return false;
