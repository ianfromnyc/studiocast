...
VfxRelightingEffect::~VfxRelightingEffect() {
  Destroy();
}
...
NvCV_Status VfxRelightingEffect::Process(studiocast::video::GpuFrame& frame, std::string* error) {
  output_ready_ = false;
  ...
  if (!frame.nvcv_gpu) {
    if (error) *error = "Relighting requires frame.nvcv_gpu (NvCVImage on GPU).";
    return -1;
  }
  if (!frame.matte_gpu) {
    if (error) *error = "Relighting requires frame.matte_gpu (Au8 matte on GPU).";
    return -1;
  }
  ...
  if (s != maxine::NVCV_SUCCESS) {
    static constexpr const char* kAltOut[] = {"dstRelit", "dstForeground", "dstFg"};
    bool ok = false;
...
bool VfxRelightingEffect::EnsureEffectCreated(std::string* error) {
  ...
  // Effect selector strings vary by SDK version/build. Try a small candidate list.
  ...
  if (s != maxine::NVCV_SUCCESS || !handle_) {
    if (error) {
      ...
      oss << ". This usually means the VFX relighting feature is not installed (run VFX install_feature.sh for relighting). ";
...
bool VfxRelightingEffect::ApplyConfigLocked(std::string* error) {
  if (!handle_) {
    if (error) *error = "Relighting effect not created.";
    return false;
  }
  ...
  if (!cfg_.hdri_path.empty()) {
    ...
    if (!ok) {
      if (error) {
        *error = "Failed to set HDRI path on relighting effect (tried selectors 'hdri', 'hdriPath', etc): ";
  ...
  // Direction/pan (best-effort; only fail if non-zero and no selector accepts it).
  const float pan = cfg_.direction_pan_degrees;
  if (pan != 0.0f) {
    static constexpr const char* kPanParams[] = {"pan", "yaw", "azimuth", "direction", "angle"};
    NvCV_Status s = static_cast<NvCV_Status>(-1);
    bool ok = false;
    ...
    if (!ok) {
      if (error) {
        *error = "Failed to set relighting direction/pan (no accepted parameter selector): ";
        *error += StatusToString(vfx_, nvcv_, s);
      }
      return false;
    }
...
bool VfxRelightingEffect::EnsureOutputImage(unsigned width, unsigned height, std::string* error) {
  if (!nvcv_ || !nvcv_->IsInitialized()) {
...
bool VfxRelightingEffect::BindMatte(const maxine::NvCVImage* matte, std::string* error) {
  if (!handle_) {
    if (error) *error = "Relighting effect not created.";
    return false;
  }
