...
class ArEyeContactEffect final : public IArFeature {
 ...
 private:
  ...
  bool EnsureStreamBound(studiocast::video::GpuFrame& frame, std::string* error);

  bool SetU32Required(studiocast::maxine::ar::NvAR_ParameterSelector sel,
                      std::uint32_t val,
                      std::string* error);
  void SetU32Optional(studiocast::maxine::ar::NvAR_ParameterSelector sel, std::uint32_t val);
  void SetF32Optional(studiocast::maxine::ar::NvAR_ParameterSelector sel, float val);
  ...
  // Bound stream used for GPU work. If `frame.cuda_stream` is supplied we use it,
  // otherwise we attempt to create one via NvAR helpers.
