...
void VfxDenoiseEffect::Destroy() {
  ...
  if (state_device_ && vfx_ && vfx_->HasCudaRuntime() && vfx_->cuda().cudaFree) {
...
std::string VfxDenoiseEffect::CudaErrorToString(maxine::vfx::VfxApi::cudaError_t err) const {
...
bool VfxDenoiseEffect::EnsureEffectCreated(std::string* error) {
  ...
  const auto& f = vfx_->f();
  const maxine::NvCV_Status st = f.NvVFX_CreateEffect(maxine::vfx::NVVFX_FX_DENOISING, &handle_);
  if (st != maxine::NVCV_SUCCESS || !handle_) {
    if (error) {
      *error = "NvVFX_CreateEffect(Denoising) failed: " + vfx_->StatusToString(st);
    }
    handle_ = nullptr;
    return false;
  }
...
bool VfxDenoiseEffect::QueryStateBytesLocked(std::size_t* out_bytes, std::string* error) {
  if (!out_bytes) return false;
  *out_bytes = 0;
  if (!handle_) {
    if (error) *error = "Denoising effect not created.";
    return false;
  }
  ...
  if (error) {
    *error = "Could not query denoise state size (no working stateSize selector).";
  }
  return false;
}
...
bool VfxDenoiseEffect::EnsureStateBufferLocked(std::string* error) {
  if (!vfx_ || !vfx_->HasCudaRuntime()) {
    if (error) *error = "CUDA runtime (libcudart) not available; denoise state allocation requires cudaMalloc.";
    return false;
  }
  const auto& c = vfx_->cuda();
  if (!c.cudaMalloc || !c.cudaFree || !c.cudaMemset) {
    if (error) *error = "CUDA runtime missing required symbols (cudaMalloc/cudaFree/cudaMemset).";
    return false;
  }
  ...
  if (want == 0) {
    if (error) *error = "Denoise state size reported as 0.";
    return false;
  }
  ...
  if (!state_device_ || state_bytes_ != want) {
    ...
    if (rc != 0 || !p) {
      if (error) {
        *error = "cudaMalloc(" + std::to_string(want) + ") failed: " + CudaErrorToString(rc);
      }
      return false;
    }
    state_device_ = p;
  ...
  if (!state_bound_) {
    const auto st = vfx_->f().NvVFX_SetObject(handle_, maxine::vfx::NVVFX_STATE, state_device_);
...
bool VfxDenoiseEffect::EnsureOutputImage(unsigned width, unsigned height, std::string* error) {
  ...
  if (out_allocated_) {
    if (out_gpu_.width == width && out_gpu_.height == height && out_gpu_.pixelFormat == want_pf &&
        out_gpu_.componentType == want_ct && out_gpu_.planar == want_layout && out_gpu_.gpuMem == want_mem) {
      return true;
    }

    if (nvcv_->f().NvCVImage_Realloc) {
      const auto st = nvcv_->f().NvCVImage_Realloc(
          &out_gpu_, width, height, want_pf, want_ct, want_layout, want_mem, /*alignment=*/0);
      if (st == maxine::NVCV_SUCCESS) return true;
    }
  ...
  const auto st = nvcv_->f().NvCVImage_Alloc(
      &out_gpu_, width, height, want_pf, want_ct, want_layout, want_mem, /*alignment=*/0);
  if (st != maxine::NVCV_SUCCESS) {
    if (error) *error = "NvCVImage_Alloc(denoise output) failed: " + std::to_string(st);
    return false;
  }
  out_allocated_ = true;
  return true;
...
maxine::NvCV_Status VfxDenoiseEffect::Process(studiocast::video::GpuFrame& frame, std::string* error) {
  output_ready_ = false;
  if (!frame.ValidDimensions() || !frame.nvcv_gpu) {
    if (error) *error = "Denoising requires frame.nvcv_gpu and valid dimensions.";
    return -1;
  }
  ...
  if (st != maxine::NVCV_SUCCESS) {
    if (error) *error = "NvVFX_Run(Denoising) failed: " + vfx_->StatusToString(st);
    return st;
  }
