...
bool VfxApi::Initialize(const std::vector<std::filesystem::path>& sdk_roots, std::string* error_out) {
    initialized_ = false;
    library_path_.clear();
    lib_.Close();
    cuda_lib_.Close();
...
bool VfxApi::InitializeFromLibraryPath(const std::filesystem::path& library_path, std::string* error_out) {
    initialized_ = false;
    library_path_.clear();
    lib_.Close();
    cuda_lib_.Close();
...
bool VfxApi::InitializeImpl(const std::vector<std::filesystem::path>& sdk_roots, std::string* error_out) {
    std::error_code ec;
    ...
    for (const auto& root : sdk_roots) {
        ...
        auto found = FindLibWithSymbol(dirs, preferred, "NvVFX_CreateEffect", util::DynLib::Scope::Global, &last);
        if (!found) {
...
bool VfxApi::InitializeFromLibraryPathImpl(const std::filesystem::path& library_path, std::string* error_out) {
    std::string err;
...
bool VfxApi::LoadSymbols(std::string* error_out) {
    ...
    if (!load_required("NvVFX_CreateEffect", &f_.NvVFX_CreateEffect) ||
        ...
        !load_required("NvVFX_SetStateObjectHandleArray", &f_.NvVFX_SetStateObjectHandleArray) ||
        ...
        !load_required("NvVFX_CudaStreamSynchronize", &f_.NvVFX_CudaStreamSynchronize) ||
