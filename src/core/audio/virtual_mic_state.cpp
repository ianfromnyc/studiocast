...
bool SaveVirtualMicState(const VirtualMicState& s, std::string* error) {
  const auto path = VirtualMicStatePath();
  if (path.empty()) {
    if (error) *error = "VirtualMicStatePath() is empty (HOME/XDG_STATE_HOME missing)";
    return false;
  }
  ...
  if (s.remap_source_module_id) out << "remap_source_module_id=" << *s.remap_source_module_id << "\n";
