...
namespace studiocast::audio {
    ...
    // Creates a virtual microphone named "studiocast_mic" backed by a null sink + remap-source.
    //
    // Processed mode (production):
    //   - Create only: module-null-sink ("studiocast_sink") + module-remap-source ("studiocast_mic")
    //   - The real-time pipeline is expected to PLAY processed audio into "studiocast_sink".
    ...
    // Legacy pass-through mode (debug-only): starts loopback from a real source -> StudioCast sink.
    //
    // This is NOT used for the processed feed (Maxine AFX) and is intentionally disabled in
    // production/release builds to avoid accidental double-routing or feedback loops.
