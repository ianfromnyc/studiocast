...
namespace studiocast::audio::pulse {
    namespace {
        std::vector<std::string> SplitTabs(const std::string &line) {
            ...
            out.push_back(cur);
            for (auto &s: out) s = util::TrimCopy(s);
            return out;
        }
    ...
    bool PactlAvailable(std::string *details) {
        ...
        if (res.exit_code != 0) {
            if (details) *details = util::TrimCopy(res.stdout_str);
            return false;
        }
        if (details) *details = util::TrimCopy(res.stdout_str);
        return true;
    ...
    std::optional<int> LoadModule(const std::string &module, const std::string &args, std::string *error) {
        ...
        if (res.exit_code != 0) {
            if (error) *error = line.empty() ? "pactl load-module failed" : line;
            return std::nullopt;
        }

        // pactl prints the module id as a number.
        if (line.empty()) {
            if (error) *error = "pactl load-module returned no module id";
            return std::nullopt;
        }
        ...
        if (!end || end == line.c_str()) {
            if (error) *error = "Failed to parse module id from pactl output: " + line;
            return std::nullopt;
        }
    ...
    std::vector<PactlModule> ListModules(std::string *error) {
        ...
        for (const auto &raw: util::SplitLines(res.stdout_str)) {
            ...
            m.name = fields[1];
            if (fields.size() >= 3) m.args = fields[2];
            out.push_back(m);
        }
        return out;
    }
    ...
    std::vector<PactlSourceInfo> ListSourcesDetailed(std::string *error) {
        ...
        auto flush = [&]() {
            if (haveCur && !cur.name.empty()) out.push_back(cur);
            cur = PactlSourceInfo{};
            haveCur = false;
            inPorts = false;
        };
        ...
        for (const auto &raw: util::SplitLines(res.stdout_str)) {
            ...
            if (StartsWith(t, "Active Port:")) {
                cur.active_port = util::TrimCopy(t.substr(std::string("Active Port:").size()));
                inPorts = false;
                continue;
            }
            ...
            if (inPorts) {
                ...
                if (colon == std::string::npos) {
                    ...
                    if (paren != std::string::npos) {
                        ...
                        if (meta.find("not available") != std::string::npos) p.available = false;
                    } else {
                        p.description = rest;
                    }

                    if (!p.name.empty()) cur.ports.push_back(p);
                    continue;
                }
            }
            ...
            if (StartsWith(t, "Description:")) {
                cur.description = util::TrimCopy(t.substr(std::string("Description:").size()));
                continue;
            }
        }
    ...
    bool SetSourcePort(const std::string &source_name, const std::string &port_name, std::string *error) {
        if (source_name.empty() || port_name.empty()) return true;

        std::string cmd = "pactl set-source-port " + source_name + " " + port_name + " 2>&1";
        auto res = util::ExecCapture(cmd);
        if (res.exit_code != 0) {
            if (error) *error = util::TrimCopy(res.stdout_str);
            return false;
    ...
    bool UpdateSinkProplist(const std::string &sink_name_or_index,
                            ...
                            std::string *error) {
        if (sink_name_or_index.empty() || kv_pairs.empty()) return true;

        std::string cmd = "pactl update-sink-proplist " + ShellQuoteSingle(sink_name_or_index);
        for (const auto &kv: kv_pairs) cmd += " " + ShellQuoteSingle(kv);
        cmd += " 2>&1";
        ...
        if (res.exit_code != 0 || LooksLikeFailure(out)) {
            if (error) *error = out.empty() ? "update-sink-proplist failed" : out;
            return false;
        }
        return true;
    }
    ...
    bool UpdateSourceProplist(const std::string &source_name_or_index,
                              const std::vector<std::string> &kv_pairs,
                              std::string *error) {
        if (source_name_or_index.empty() || kv_pairs.empty()) return true;

        std::string cmd = "pactl update-source-proplist " + ShellQuoteSingle(source_name_or_index);
        for (const auto &kv: kv_pairs) cmd += " " + ShellQuoteSingle(kv);
        cmd += " 2>&1";
        ...
        if (res.exit_code != 0 || LooksLikeFailure(out)) {
            if (error) *error = out.empty() ? "update-source-proplist failed" : out;
            return false;
        }
        return true;
    }
