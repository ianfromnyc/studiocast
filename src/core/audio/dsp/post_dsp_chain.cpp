...
PostDspChain::Biquad PostDspChain::MakeHighShelfBiquad(int sample_rate,
  ...
  if (sample_rate <= 0) return bq;
  if (freq_hz <= 0.0f) return bq;
  ...
  const double w0 = (2.0 * kPi) * static_cast<double>(f) / static_cast<double>(fs);
  ...
  const double b0 = static_cast<double>(A) * (Ap1 + Am1 * cosw0 + 2.0 * sqrtA * alpha);
  const double b1 = -2.0 * static_cast<double>(A) * (Am1 + Ap1 * cosw0);
  const double b2 = static_cast<double>(A) * (Ap1 + Am1 * cosw0 - 2.0 * sqrtA * alpha);
...
bool PostDspChain::Configure(int sample_rate, std::uint32_t channels, std::string* error) {
  if (error) error->clear();
  if (sample_rate <= 0) {
    if (error) *error = "PostDspChain: sample_rate must be > 0";
    return false;
  }
  if (channels == 0) {
    if (error) *error = "PostDspChain: channels must be > 0";
    return false;
  }
...
void PostDspChain::RecomputePresenceShelfIfNeeded() {
  ...
  const bool changed = (presence_cfg_.enabled != presence_cfg_cached_.enabled) ||
  ...
  presence_biquad_ = MakeHighShelfBiquad(sample_rate_,
                                        presence_cfg_.freq_hz,
                                        presence_cfg_.gain_db,
                                        presence_cfg_.slope);
}

void PostDspChain::ProcessInPlace(float* interleaved, std::uint32_t frames, std::uint32_t channels) {
  ...
  const bool presence_on = presence_cfg_.enabled && (std::fabs(presence_cfg_.gain_db) >= 0.001f);
  ...
  if (limiter_cfg_.enabled) {
    ...
    const float frame_sec = static_cast<float>(frames) / static_cast<float>(sample_rate_);
    const float attack_sec = std::max(0.0001f, limiter_cfg_.attack_ms / 1000.0f);
    const float release_sec = std::max(0.0001f, limiter_cfg_.release_ms / 1000.0f);
    const float alpha_attack = std::exp(-frame_sec / attack_sec);
    const float alpha_release = std::exp(-frame_sec / release_sec);

    const float alpha = (target_gain < limiter_gain_) ? alpha_attack : alpha_release;
