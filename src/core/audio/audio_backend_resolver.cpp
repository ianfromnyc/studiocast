...
bool AnyMicrophoneEffectRequested(
  ...
  return fx.microphone.studio_voice_enabled ||
...
bool AnySpeakerEffectRequested(
  ...
  return fx.speaker.noise_removal_enabled ||
         fx.speaker.room_echo_removal_enabled;
}
...
ResolveAudioBackend(const studiocast::audio::effects::BroadcastAudioEffects &fx,
  ...
  case Pref::kOpenSource: {
    ...
    out.used_fallback = true;
    out.note =
        "Open-source audio requested but unavailable; using pass-through.";
  ...
  case Pref::kAuto: {
    ...
    std::string r;
    if (!avail.maxine_reason.empty())
      r = FirstLine(avail.maxine_reason);
    if (r.empty() && !avail.open_source_reason.empty())
      r = FirstLine(avail.open_source_reason);
    if (!r.empty())
      out.note += "\n" + r;
    return out;
  }
