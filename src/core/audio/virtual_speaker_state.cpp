...
std::optional<int> GetOptInt(const std::map<std::string, std::string>& kv, const char* key) {
  auto it = kv.find(key);
...
std::optional<std::string> GetOptString(const std::map<std::string, std::string>& kv, const char* key) {
  auto it = kv.find(key);
  if (it == kv.end()) return std::nullopt;
  const auto v = util::TrimCopy(it->second);
  if (v.empty()) return std::nullopt;
...
bool SaveVirtualSpeakerState(const VirtualSpeakerState& s, std::string* error) {
  const auto path = VirtualSpeakerStatePath();
  if (path.empty()) {
    if (error) *error = "VirtualSpeakerStatePath() is empty (HOME/XDG_STATE_HOME missing)";
    return false;
  }
  ...
  if (ec) {
    if (error) *error = "Failed to create state dir: " + ec.message();
    return false;
  }

  std::ofstream out(path, std::ios::out | std::ios::trunc);
  if (!out.is_open()) {
    if (error) *error = "Failed to write state file: " + path.string();
    return false;
  }

  out << "# StudioCast virtual speakers runtime state (module ids)\n";
  if (s.null_sink_module_id) out << "null_sink_module_id=" << *s.null_sink_module_id << "\n";
  if (s.loopback_module_id) out << "loopback_module_id=" << *s.loopback_module_id << "\n";
  if (s.loopback_target_sink_name) out << "loopback_target_sink_name=" << *s.loopback_target_sink_name << "\n";
...
bool ClearVirtualSpeakerState(std::string* error) {
  ...
  fs::remove(path, ec);
  if (ec) {
    if (error) *error = "Failed to remove state file: " + ec.message();
    return false;
  }
  return true;
}
