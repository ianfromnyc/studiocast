...
bool RequireRangeInt(std::string_view path, int v, int lo, int hi, std::string* error) {
    if (v < lo || v > hi) {
        return Fail(error,
                    std::string(path) + " must be in [" + std::to_string(lo) + ", " + std::to_string(hi) + "]");
    }
    return true;
}
...
bool ParseRootObject(const Value& root,
    ...
    if (!obj) return Fail(error, "root must be a JSON object");
    *out = obj;
    (void)warnings;
    return true;
}

bool ValidateMicExclusivity(const BroadcastAudioEffects& fx, std::string* error) {
    if (fx.microphone.studio_voice_enabled &&
        (fx.microphone.noise_removal_enabled || fx.microphone.room_echo_removal_enabled)) {
        return Fail(error,
                    "microphone.studio_voice_enabled is mutually exclusive with microphone.noise_removal_enabled and microphone.room_echo_removal_enabled");
    }
    return true;
}
...
bool ParseBroadcastAudioEffectsJson(const studiocast::util::json::Value& root,
                                   BroadcastAudioEffects* out,
                                   const BroadcastAudioEffectsJsonParseOptions& options,
                                   std::vector<std::string>* warnings,
                                   std::string* error) {
    ...
    *out = BroadcastAudioEffects{};
    ...
    if (const auto* mic = GetObj(*obj, "", "microphone", error)) {
        ...
        if (found) out->microphone.noise_removal_enabled = en;

        en = out->microphone.room_echo_removal_enabled;
        if (!TryGetBool(*mic, "microphone", "room_echo_removal_enabled", &found, &en, error)) return false;
        if (found) out->microphone.room_echo_removal_enabled = en;

        int strength = out->microphone.strength;
        if (!TryGetInt(*mic, "microphone", "strength", &found, &strength, error)) return false;
        if (found) {
            if (!RequireRangeInt("microphone.strength", strength, kStrengthMin, kStrengthMax, error)) return false;
...
bool ParseBroadcastAudioEffectsJsonText(const std::string& jsonText,
                                       BroadcastAudioEffects* out,
                                       const BroadcastAudioEffectsJsonParseOptions& options,
                                       std::vector<std::string>* warnings,
                                       std::string* error) {
