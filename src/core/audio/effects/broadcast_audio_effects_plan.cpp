...
PlannedBroadcastAecEffect PlanAec(const BroadcastMicrophoneEffects::Aec& aec,
    ...
    if (aec.reference_source.empty()) {
        out.enabled = false;
        out.reference_source.clear();
        out.reason = "microphone.aec.enabled requested but microphone.aec.reference_source is empty";
        return out;
    }

    if (!in.available_pulse_sources.empty() && !Contains(in.available_pulse_sources, aec.reference_source)) {
        out.enabled = false;
        out.reference_source.clear();
        out.reason = "microphone.aec.reference_source not available: " + aec.reference_source;
        return out;
    }
...
PlannedBroadcastSuperresEffect PlanSuperres(const char* scope,
                                           const BroadcastMicrophoneEffects::Superres& sr,
                                           const BroadcastAudioEffectsPlanInputs& in) {
    ...
    if (!in.float32_pcm) {
        out.enabled = false;
        out.reason = std::string(scope) + " superres requested but pipeline is not float32 PCM";
        return out;
    }
    out.enabled = true;
    out.reason.clear();
...
PlannedBroadcastSuperresEffect PlanSuperres(const char* scope,
                                           const BroadcastSpeakerEffects::Superres& sr,
                                           const BroadcastAudioEffectsPlanInputs& in) {
    PlannedBroadcastSuperresEffect out;
    out.mode = sr.mode;
    if (!sr.enabled) {
...
PlannedBroadcastAudioEffects PlanBroadcastAudioEffects(const BroadcastAudioEffects& fx,
    ...
    out.microphone_superres = PlanSuperres("microphone", fx.microphone.superres, in);
