...
studiocast::maxine::CUresult
LoadPtxModule(studiocast::maxine::CudaDriverApi *cuda, const char *ptx,
              studiocast::maxine::CUmodule *module, std::string *jit_log) {
  if (jit_log)
    jit_log->clear();
  if (module)
    *module = nullptr;
  ...
  if (f.cuModuleLoadDataEx) {
    ...
    const auto rc = f.cuModuleLoadDataEx(
        module, ptx, static_cast<unsigned int>(sizeof(opts) / sizeof(opts[0])),
        opts, vals);
    if (jit_log) {
  ...
  return f.cuModuleLoadData(module, ptx);
}

std::string OpenCudaVbPtxCompat82() {
  std::string ptx(kOpenCudaVbPtx);
  const std::string from = "\n.version 8.5\n";
  const std::string to = "\n.version 8.2\n";
  const auto pos = ptx.find(from);
...
bool EnsureKernelsLoaded(studiocast::maxine::CudaDriverApi *cuda,
  ...
  if (rc != studiocast::maxine::CUDA_SUCCESS) {
    const std::string primary_error =
        ...
        (jit_log.empty() ? std::string() : "\n" + jit_log);

    const std::string compat_ptx = OpenCudaVbPtxCompat82();
    if (!compat_ptx.empty()) {
      ...
      if (compat_rc == studiocast::maxine::CUDA_SUCCESS) {
        ...
        *error_out = "cuModuleLoadData(open_cuda_vb_kernels) failed: " +
                     cuda->StatusToString(compat_rc) +
                     "\n" + primary_error +
                     "\ncompatibility PTX 8.2 load failed: " +
                     cuda->StatusToString(compat_rc);
        if (!compat_jit_log.empty())
          *error_out += "\n" + compat_jit_log;
      }
    }
  }
