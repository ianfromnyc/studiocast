...
extern "C" __global__ void resize_bilinear_f32_1(const void* srcPtr,
                                                  unsigned int srcPitchBytes,
                                                  unsigned int srcW,
                                                  unsigned int srcH,
                                                  void* dstPtr,
                                                  unsigned int dstPitchBytes,
                                                  unsigned int dstW,
                                                  unsigned int dstH) {
  const int x = static_cast<int>(blockIdx.x * blockDim.x + threadIdx.x);
  const int y = static_cast<int>(blockIdx.y * blockDim.y + threadIdx.y);
  if (x >= static_cast<int>(dstW) || y >= static_cast<int>(dstH)) return;
  ...
  const auto* row0 = reinterpret_cast<const float*>(srcBase + static_cast<size_t>(y0) * srcPitchBytes);
  const auto* row1 = reinterpret_cast<const float*>(srcBase + static_cast<size_t>(y1) * srcPitchBytes);
  ...
  auto* dstRow = reinterpret_cast<float*>(dstBase + static_cast<size_t>(y) * dstPitchBytes);
  dstRow[x] = v;
}

extern "C" __global__ void box_blur_h_u8x3(const void* srcPtr,
                                           unsigned int srcPitchBytes,
                                           unsigned int w,
                                           unsigned int h,
                                           void* dstPtr,
  ...
  const int x = static_cast<int>(blockIdx.x * blockDim.x + threadIdx.x);
  const int y = static_cast<int>(blockIdx.y * blockDim.y + threadIdx.y);
  if (x >= static_cast<int>(w) || y >= static_cast<int>(h)) return;

  radius = radius < 0 ? 0 : radius;
  const int count = radius * 2 + 1;

  const auto* srcBase = static_cast<const unsigned char*>(srcPtr) + static_cast<size_t>(y) * srcPitchBytes;
  ...
  auto* dstBase = static_cast<unsigned char*>(dstPtr) + static_cast<size_t>(y) * dstPitchBytes;
  const int off = x * 3;
...
extern "C" __global__ void box_blur_v_u8x3(const void* srcPtr,
                                           unsigned int srcPitchBytes,
                                           unsigned int w,
                                           unsigned int h,
                                           void* dstPtr,
  ...
  for (int dy = -radius; dy <= radius; ++dy) {
    ...
    const auto* row = static_cast<const unsigned char*>(srcPtr) + static_cast<size_t>(yy) * srcPitchBytes;
    const int off = x * 3;
  ...
  auto* dstRow = static_cast<unsigned char*>(dstPtr) + static_cast<size_t>(y) * dstPitchBytes;
  const int off = x * 3;
...
extern "C" __global__ void box_blur_h_f32_1(const void* srcPtr,
                                            unsigned int srcPitchBytes,
                                            unsigned int w,
                                            unsigned int h,
                                            void* dstPtr,
  ...
  const auto* srcBaseU8 = static_cast<const unsigned char*>(srcPtr) + static_cast<size_t>(y) * srcPitchBytes;
  ...
  auto* dstBaseU8 = static_cast<unsigned char*>(dstPtr) + static_cast<size_t>(y) * dstPitchBytes;
  auto* dstRow = reinterpret_cast<float*>(dstBaseU8);
  dstRow[x] = sum * inv;
}

extern "C" __global__ void box_blur_v_f32_1(const void* srcPtr,
                                            unsigned int srcPitchBytes,
                                            unsigned int w,
                                            unsigned int h,
                                            void* dstPtr,
  ...
  for (int dy = -radius; dy <= radius; ++dy) {
    const int yy = clampi(y + dy, 0, static_cast<int>(h) - 1);
    const auto* rowU8 = static_cast<const unsigned char*>(srcPtr) + static_cast<size_t>(yy) * srcPitchBytes;
    const auto* row = reinterpret_cast<const float*>(rowU8);
    sum += row[x];
  }

  auto* dstU8 = static_cast<unsigned char*>(dstPtr) + static_cast<size_t>(y) * dstPitchBytes;
  auto* dstRow = reinterpret_cast<float*>(dstU8);
  dstRow[x] = sum * inv;
}

extern "C" __global__ void composite_alpha_u8x3_bg(const void* fgPtr,
                                                    unsigned int fgPitchBytes,
                                                    const void* bgPtr,
                                                    unsigned int bgPitchBytes,
                                                    const void* alphaPtr,
                                                    unsigned int alphaPitchBytes,
                                                    unsigned int w,
                                                    unsigned int h,
                                                    void* outPtr,
  ...
  const auto* fgRow = static_cast<const unsigned char*>(fgPtr) + static_cast<size_t>(y) * fgPitchBytes;
  const auto* bgRow = static_cast<const unsigned char*>(bgPtr) + static_cast<size_t>(y) * bgPitchBytes;

  const auto* aRowU8 = static_cast<const unsigned char*>(alphaPtr) + static_cast<size_t>(y) * alphaPitchBytes;
  const auto* aRow = reinterpret_cast<const float*>(aRowU8);
  const float a = clamp01(aRow[x]);
  const float ia = 1.0f - a;

  auto* outRow = static_cast<unsigned char*>(outPtr) + static_cast<size_t>(y) * outPitchBytes;
  const int off = x * 3;
...
extern "C" __global__ void composite_alpha_u8x3_solid(const void* fgPtr,
                                                       unsigned int fgPitchBytes,
                                                       const void* alphaPtr,
                                                       unsigned int alphaPitchBytes,
                                                       unsigned int w,
