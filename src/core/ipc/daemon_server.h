...
//   - The handler is called once per request line and must return the response line
...
class DaemonServer final {
 public:
  ...
  bool Start(const std::filesystem::path& socket_path, Handler handler, std::string* error);
  void Stop();
