...
std::string ToString(GpuSelectMode m) {
  switch (m) {
    case GpuSelectMode::Auto: return "auto";
    case GpuSelectMode::Index: return "index";
    case GpuSelectMode::Uuid: return "uuid";
  }
  return "auto";
}
...
GpuSelectMode ParseGpuSelectMode(const std::string& raw) {
  ...
  if (s == "auto") return GpuSelectMode::Auto;
  if (s == "index") return GpuSelectMode::Index;
  if (s == "uuid") return GpuSelectMode::Uuid;
...
bool SaveSettings(const Settings& s, std::string* error) {
  const auto path = SettingsPath();
  if (path.empty()) {
    if (error) *error = "SettingsPath() is empty (HOME/XDG_CONFIG_HOME not available).";
    return false;
  }
  ...
  if (!out.is_open()) {
    if (error) *error = "Failed to open settings file for writing: " + path.string();
    return false;
  }
