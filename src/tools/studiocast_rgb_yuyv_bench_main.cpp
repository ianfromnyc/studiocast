...
void Usage(const char *argv0) {
  std::cout << "StudioCast RGB24 -> YUYV Benchmark\n\n"
            << "Usage:\n"
            << "  " << argv0 << " [--iterations N] [--warmup N] [--csv]\n\n"
            << "Backends: original-scalar, current-scalar, libyuv, ssse3, "
               "avx2, selected\n"
...
int main(int argc, char **argv) {
  ...
  std::cout << "StudioCast RGB24 -> YUYV Benchmark\n";
  std::cout << "Selected production backend: "
  ...
  if (csv) {
    std::cout << "backend,width,height,avg_ms,p95_ms,p99_ms,min_ms,max_ms,"
                 "checksum\n";
  } else {
  ...
  for (BenchBackend backend : backends) {
    ...
    for (const SizeCase &size : sizes) {
      ...
      if (csv) {
        std::cout << BackendName(backend) << "," << size.width << ","
